...
self.addEventListener("fetch", (event) => {
  ...
  // Always fetch latest core bundles (ensures you see UI/login changes immediately)
  if (url.origin === self.location.origin && (url.pathname.endsWith("/app.css") || url.pathname.endsWith("/app.js"))) {
    event.respondWith((async () => {
      const cache = await caches.open(CACHE_NAME);
      try{
        const fresh = await fetch(req, { cache: "no-store" });
        if (fresh && fresh.ok) cache.put(normalizedRequest(req), fresh.clone());
        return fresh;
      }catch(_){
        const cached = await cache.match(normalizedRequest(req));
