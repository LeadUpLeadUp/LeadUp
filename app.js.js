...
function seedDemoDataOnce(){
  ...
  try{
    ...
    try{ localStorage.setItem(LS_KEY, JSON.stringify(appState)); }catch(e2){}
...
function setView(view){
  ...
  if(view === "esign"){
    if(typeof window.openESignModal === "function"){ window.openESignModal(); }
    else{ toast("חתימות: בפיתוח", "warn"); }
    return;
  }
...
function renderCustomerPolicies(){
  ...
  tb.querySelectorAll("button[data-pol-del]").forEach(b=>{
    b.onclick = ()=>{
      ...
      const removed = lead.policies[i];
      lead.policies.splice(i,1);
      addEvent("policy", `נמחקה פוליסה: ${removed?.name || ""}`, { leadId: lead.id });
      saveState();
...
function renderCustomerHistory(){
  ...
  const events = (appState.events||[]).filter(ev=>ev?.meta?.leadId===lead.id).slice().reverse();
  if(!events.length){
...
function wireCustomerView(){
  ...
  el("btnPolicyAdd")?.addEventListener("click", ()=>{
    ...
    addEvent("policy", `נוספה פוליסה: ${name}`, { leadId: lead.id });
    saveState();
    renderCustomerPolicies();
    // update totals
    const total = _cdComputePremiumTotal(lead);
  ...
  const signNow = ()=>{
    const lead = getCustomerLead();
    if(!lead) return;
    addEvent("sign", "נשלח מסמך לחתימה (דמו)", { leadId: lead.id });
    saveState();
...
function toast(msg, type){
  ...
  // Normalize aliases used around the app
  guess = guess.toLowerCase();
  if(guess==="ok") guess="success";
  if(guess==="bad" || guess==="danger" || guess==="err") guess="error";
  if(guess==="warning") guess="warn";
...
function importJSON(){
  ...
  inp.onchange = () => {
    ...
    toast("ייבוא נתונים הצליח ✅", "success");
...
function getSelectedLead(){
  ...
  if(typeof window.openESignModal!=="function"){
    window.openESignModal = ()=> toast("חתימות: בפיתוח", "warn");
  }
