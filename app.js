...
async function gsPull(){
  ...
  if(!data || data.ok !== true || !data.payload) throw new Error("לא התקבל payload תקין");
  // Replace state safely
  const next = data.payload;
  if(!next.leads || !next.tasks || !next.events) throw new Error("מבנה נתונים לא תקין");
  appState.leads = next.leads;
  appState.tasks = next.tasks;
  appState.events = next.events;
...
async function gsPush(){
  ...
  const body = JSON.stringify({
    action: "put",
    payload: appState,
    meta: { app:"LEADUP", version:"20260131" }
  });
...
function updateGsUI(){
  ...
  if(appCfg.mode === "sheets"){
    gsSetStatus(appCfg.lastOkAt ? "מחובר (נבדק לאחרונה)" : "מוכן לחיבור", true);
  }else{
...
function saveState(){
  ...
  // Persist locally (handle quota / blocked storage)
  let localOk = false;
  try{
    ...
    // Try to shrink heavy arrays and retry (events/history can grow fast)
    try{
      if(Array.isArray(appState.events) && appState.events.length > 250){
        appState.events = appState.events.slice(0, 250);
      }
      localStorage.setItem(LS_KEY, JSON.stringify(appState));
      localOk = true;
      toast("האחסון התמלא—קיצרתי היסטוריה ושמרתי ✅", "warn");
    }catch(e2){
      // Last resort: session backup (survives refresh, not full browser close)
      try{ sessionStorage.setItem(LS_KEY, JSON.stringify(appState)); }catch(_){}
      toast("לא הצלחתי לשמור בדפדפן (אחסון חסום/מלא). מומלץ לבצע ייצוא JSON.", "bad");
    }
  }
...
function openLeadForm(leadId=null){
  ...
  form.addEventListener("submit", (e)=>{
    ...
    const _ok = saveState();
    // SaveState already re-renders; keep extra render as fallback only
    try{ renderAll(); }catch(err){ console.error("renderAll failed", err); }
    if(_ok){
      toast("ליד נשמר בהצלחה ✅", "success");
    }else{
      toast("הליד נוסף למסך אבל לא נשמר בדפדפן. בצע גיבוי JSON או הפעל Sheets.", "warn");
    }
    setTimeout(()=>{
...
function openClientDossier(id){
  ...
  // Close
  el("clientDossierClose")?.addEventListener("click", closeClientDossier, { once:true });

  // Add policy/doc/event
  el("cd_addPolicy")?.addEventListener("click", ()=>{
    const lead = _cdGetLead();
    if(!lead) return;
    const type = prompt("סוג ביטוח (למשל רכב/דירה/בריאות):", "") ?? null;
    if(type===null) return;
    const company = prompt("חברת ביטוח:", "") ?? null;
    if(company===null) return;
    const premium = prompt("פרמיה חודשית (מספר):", "") ?? null;
    if(premium===null) return;
    const note = prompt("הערה (אופציונלי):", "") ?? null;
    if(note===null) return;

    const p = {
      id: uid("pol"),
      type: String(type||"").trim() || "ביטוח",
      company: String(company||"").trim(),
      premium: Number(String(premium||"").replace(/[^\d.]/g,"")),
      note: String(note||"").trim(),
      createdAt: _cdNowISO()
    };
    lead.policies = [...(lead.policies||[]), p];
  ...
  el("cd_addDoc")?.addEventListener("click", ()=>{
    const lead = _cdGetLead();
    if(!lead) return;
  ...
  el("clientDossierAddNote")?.addEventListener("click", addEventFromDossier, { once:true });
  el("cd_addEvent2")?.addEventListener("click", addEventFromDossier, { once:true });
...
/* ===== Login Gate Logic (restored) =====
   Fix: login buttons were not wired, so entering password couldn't unlock the app.
   - Default password: 3316 (emergency fallback)
   - Remembers login per-tab (sessionStorage) so refresh doesn't ask again.
*/
(function(){
  // ✅ Default password (emergency fallback)
  // If a custom password was saved earlier in localStorage, BOTH it and this
  // default will work — so you can always get back in.
  const DEFAULT_PASSWORD = "3316";            // <-- סיסמת ברירת מחדל
  const PASS_KEY = "leadup_password";         // optional: store custom password
  const AUTH_KEY = "leadup_auth_ok";          // session flag
  ...
  function getPassword(){
    try{
      ...
      return (stored && stored.trim()) ? stored.trim() : DEFAULT_PASSWORD;
    }catch(_){
  ...
  function attempt(){
    ...
    // Accept either stored password OR default fallback
    if(val === DEFAULT_PASSWORD || val === getPassword()){
      setError("");
      unlock();
      return;
    }
  ...
  // If already authenticated in this tab, unlock immediately
  try{
  ...
  window.LEADUP_AUTH = { unlock, lock, setPassword:(p)=>{
    try{
      localStorage.setItem(PASS_KEY, String(p||"").trim());
      return true;
