...
`;
  ...
  function openPrintView(proposal){
      ...
      return `
        <div class="ibox">
          ...
          <div class="igrid">
            ...
            <div><b>BMI:</b> ${ (d.heightCm && d.weightKg) ? esc(((Number(d.weightKg)||0)/Math.pow((Number(d.heightCm)||0)/100,2)).toFixed(1))) : '—' }</div>
    ...
    function policiesTable(kind){
      ...
      const head = isNew
        ? `<tr>
            ...
            <th style="width:16%">מוצר</th>
            <th style="width:10%">פרמיה</th>
            <th style="width:10%">הנחה מדורגת</th>
            <th style="width:16%">סוגי הנחה</th>
            <th style="width:22%">פירוט הנחה</th>
            <th style="width:12%">אחרי הנחה</th>
          </tr>`
        : `<tr>
            <th style="width:30%">חברה</th>
            <th style="width:48%">מוצר</th>
            <th style="width:22%">פרמיה</th>
          </tr>`;
      ...
      const blocks = insuredList.map(ins => {
        ...
        let totalAfter = 0;
        rows.forEach(x => {
          totalPrem += Number(x.premium)||0;
          if(isNew) totalAfter += Number(premiumAfterDiscount(x))||0;
        });
        const totalDiscount = isNew ? (totalPrem - totalAfter) : 0;
        ...
        return `
          <div class="ibox">
            ...
            <table class="tbl">
              ...
              <tbody>
                ${rows.map(x => {
                  const prem = Number(x.premium)||0;
                  if(isNew){
                    const d = x.discount || null;
                    const tier = d && d.tier ? String(d.tier) : '—';
                    const types = d && Array.isArray(d.types) && d.types.length ? d.types.join(' • ') : '—';
                    const note = d && d.note && String(d.note).trim() ? String(d.note).trim() : '—';
                    const afterVal = Number(premiumAfterDiscount(x))||0;
                    return `<tr>
                      <td>${esc(x.company)}</td>
                      <td>${esc(x.product)}</td>
                      <td>${money(prem)}</td>
              ...
              <tfoot>
                ${isNew ? `
                  <tr>
                    <td colspan="2" style="font-weight:700">סה״כ</td>
                    <td style="font-weight:700">${money(totalPrem)}</td>
                    <td colspan="3" style="font-weight:700">סה״כ הנחות: ${money(totalDiscount)}</td>
                    <td style="font-weight:700">${money(totalAfter)}</td>
                  </tr>
                ` : `
                  <tr>
                    <td colspan="2" style="font-weight:700">סה״כ</td>
    ...
    // KPI totals (for operational view)
    const totalOldPremium = (p.oldPolicies||[]).reduce((s,x)=>s + (Number(x.premium)||0), 0);
    const totalNewPremium = (p.newPolicies||[]).reduce((s,x)=>s + (Number(x.premium)||0), 0);
    const totalNewAfter = (p.newPolicies||[]).reduce((s,x)=>s + (Number(premiumAfterDiscount(x))||0), 0);
    const totalDiscount = totalNewPremium - totalNewAfter;

    const kpiHtml = `
      <div class="kpis">
        <div class="kpi"><b>מבוטחים</b><span>${insuredList.length}</span></div>
        <div class="kpi"><b>פרמיה קיימת (חודשית)</b><span>${money(totalOldPremium)}</span></div>
        <div class="kpi"><b>פרמיה חדשה לפני הנחות</b><span>${money(totalNewPremium)}</span></div>
        <div class="kpi"><b>סה״כ הנחות / אחרי הנחה</b><span>${money(totalDiscount)} • ${money(totalNewAfter)}</span></div>
      </div>
    `;
    ...
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          body{font-family:Arial, sans-serif; margin:0; color:#121319; font-size:12px}
          ...
          .tbl{width:100%; border-collapse:collapse; margin:8px 0 0; table-layout:fixed}
          th,td{border:1px solid #ddd; padding:6px 7px; text-align:right; font-size:12px; vertical-align:top; word-wrap:break-word}
          th{background:#f7f6f2; font-weight:700}
          tfoot td{background:#fbfaf6}
          .ibox{border:1px solid #ddd; padding:10px; border-radius:10px; margin:10px 0}
          .ititle{font-weight:800; margin-bottom:6px}
          .igrid{display:grid; grid-template-columns:repeat(4, minmax(0,1fr)); gap:6px 12px}
          .muted{color:#666; font-size:11px}
          .inotes{margin-top:6px; padding-top:6px; border-top:1px dashed #ddd}
          .kpis{display:grid; grid-template-columns:repeat(4, minmax(0,1fr)); gap:8px; margin-top:8px}
          .kpi{border:1px solid #ddd; border-radius:10px; padding:8px; background:#fff}
          .kpi b{display:block; font-size:11px; color:#666; margin-bottom:2px}
          .kpi span{font-size:14px; font-weight:800}
          @media print{
            body{margin:0}
          }
        </style>
      ...
      <body>
        ...
        <h3>סיבת החלפה פוליסה בתיק הביטוחי</h3>
        ${
          (Array.isArray(p.oldPolicySwitchReasons) && p.oldPolicySwitchReasons.length)
            ? ('<ul>' + p.oldPolicySwitchReasons.map(k => {
                const r = OLD_POLICY_SWITCH_REASONS.find(x => x.key === k);
