...
function policyPremium(policy){
  ...
  // זהה ללוגיקת הדוח התפעולי במערכת הראשית:
  // הפרמיה שהנציג מזין היא כבר "פרמיה חודשית לאחר הנחה".
  // אם זו פוליסת בריאות עם מחלות קשות/סרטן כ-addon — מוסיפים את פרמיות ה-addon בנפרד.
  const direct = num(policy.premiumAfterDiscountValue ?? policy.premiumAfterDiscount);

  let base = direct > 0
    ? direct
    : num(policy.premiumMonthly ?? policy.monthlyPremium ?? policy.premium ?? policy.premiumBefore ?? policy.monthlyPremiumAfterDiscount ?? policy.finalMonthlyPremium);

  // בפוליסה מרובת מבוטחים, לעיתים הבסיס נשמר כ-premiumPerInsured.
  if(base <= 0 && policy.premiumPerInsured && typeof policy.premiumPerInsured === "object"){
    base = Object.values(policy.premiumPerInsured).reduce((sum, value) => sum + num(value), 0);
  }

  let addonSum = 0;
  if(safe(policy.type || policy.product) === "בריאות" && policy.healthAddonPremiums && typeof policy.healthAddonPremiums === "object"){
    Object.values(policy.healthAddonPremiums).forEach(byInsured => {
      if(byInsured && typeof byInsured === "object"){
        Object.values(byInsured).forEach(value => { addonSum += num(value); });
      }
    });
  }

  return Math.max(0, Math.round((base + addonSum) * 100) / 100);
}

function collectNewPolicies(row){
  const payload = parsePayload(row);
  const policies = [];
  const push = (policy, source = "") => {
    if(policy && typeof policy === "object") policies.push({ ...policy, __source: source });
  };

  (Array.isArray(payload.newPolicies) ? payload.newPolicies : []).forEach(p => push(p, "payload.newPolicies"));
  (Array.isArray(payload?.operational?.newPolicies) ? payload.operational.newPolicies : []).forEach(p => push(p, "payload.operational.newPolicies"));
  ...
  insureds.forEach((insured, insuredIndex) => {
    const label = safe(insured?.label || insured?.type || `מבוטח ${insuredIndex + 1}`);
    const np = Array.isArray(insured?.data?.newPolicies) ? insured.data.newPolicies : [];
    np.forEach(policy => push({ ...policy, __insuredLabel: label }, `insureds.${insuredIndex}.newPolicies`));
  });

  // מניעת כפילויות בין payload.newPolicies לבין insureds[].data.newPolicies.
  const seen = new Set();
  return policies.filter((policy, index) => {
    const ids = Array.isArray(policy.insuredIds) ? policy.insuredIds.join("|") : safe(policy.insuredId || policy.__insuredLabel);
    const key = safe(policy.id || policy.policyId || policy.policyNumber) || [
      safe(policy.company),
      safe(policy.type || policy.product),
      safe(policy.startDate),
      ids,
      String(policyPremium(policy))
    ].join("::");
    const finalKey = key || `idx_${index}`;
...
function proposalPremium(row){
  ...
  const policiesTotal = policies.reduce((sum, policy) => sum + policyPremium(policy), 0);

  // אם יש פוליסות חדשות — הן מקור האמת. לא לוקחים summary כדי לא לספור כפול / נתון ישן.
  if(policies.length && policiesTotal > 0) return Math.round(policiesTotal * 100) / 100;

  const payload = parsePayload(row);
  const direct = num(
    row.monthlyPremium ?? row.monthly_premium ?? row.premium ?? row.totalPremium ?? row.total_premium ??
    payload.totalPremiumAfterDiscount ?? payload.totalMonthlyPremium ?? payload.monthlyPremium ?? payload.premium ??
    payload.summary?.totalPremiumAfterDiscount ?? payload.summary?.totalMonthlyPremium ?? payload.operationalReport?.totalPremium
  );

  return Math.max(0, Math.round(direct * 100) / 100);
}
...
function dailyChampion(){
  const today = agentStats("today").filter(agent => agent.premium > 0 || agent.proposals > 0);
  return today.length ? today[0] : null;
}
function championKpiHtml(agent){
  if(!agent) return kpiHtml("מצטיין יומי", "—", "🏆", "אין מכירות היום עדיין");
  return `<div class="glass kpiCard kpiCard--champion"><div class="championRibbon">TOP TODAY</div><div class="kpiTop"><div><div class="kpiTitle">מצטיין יומי</div><div class="kpiValue kpiValue--name">${agent.name}</div></div><div class="kpiIcon">🏆</div></div><div class="kpiChange">${money(agent.premium)} · ${agent.proposals} הצעות היום</div></div>`;
}
function render(){
  ...
  if(cur.role==="ops"){
    ...
    $("kpiGrid").innerHTML=[
      ...
      cur.role==="manager" ? championKpiHtml(dailyChampion()) : ""
    ].join("");
  }
