...
const state = {
  ...
  function updateSide(){
    ...
    safeText(stageBadge, stepName);
    safeText(badgeStage, stepName);
  ...
  function setStep(step){
    ...
    steps.forEach((b, i)=>{
      ...
      b.classList.toggle("done", idx >= 0 && i < idx);
    });
  ...
  function renderCancellationsSummary(){
    const entities = getInsuredEntities();
    const sections = entities.map(ent=>{
      const cancels = getCancellationsForKey(ent.key) || [];
      const rows = (cancels.length ? cancels : []).map((p)=>`
        <div class="cancelRow" style="margin-top:10px">
          <div class="formGrid4">
            <div class="field"><div class="label">חברה</div><div class="chip">${esc(p.company || "—")}</div></div>
            <div class="field"><div class="label">שם פוליסה / ביטוח</div><div class="chip">${esc(p.product || "—")}</div></div>
            <div class="field"><div class="label">פרמיה</div><div class="chip mono">${esc(String(num(p.premiumPaid)))}</div></div>
            <div class="field"><div class="label">ביטול</div><div class="chip">${esc(p.cancelType || "מלא")}</div></div>
          </div>
        </div>
      `).join("");

      const total = sumCancelPremium(cancels);
      return `
        <div class="card">
          <div class="cardTop">
            <div>
              <div class="cardTitle">${esc(ent.label)}</div>
              <div class="cardMeta">פוליסות נגדיות לביטול</div>
            </div>
            <span class="pill">סה״כ פרמיות: <span class="mono">${esc(String(total))}</span> ₪</span>
          </div>
          <div style="margin-top:10px">
            ${rows || `<div class="chip">אין פוליסות לביטול</div>`}
          </div>
        </div>
      `;
    }).join("");
    return sections || `<div class="chip">אין נתונים</div>`;
  }
...
function renderInsuredComparisonSummary(){
    ...
    app.innerHTML = `
      <div class="sectionHead">
        <div class="sectionLeft">
          <div class="sectionTitle">פרטי מבוטחים</div>
          <div class="sectionSub">מלא פרטים בצורה מסודרת: ראשי, משני וילדים. לאחר מכן עוברים לביטולים.</div>
        </div>
        <div class="sectionRight">
          ${`<span class="kpi">ילדים: <span class="mono">${(state.case.children||[]).length}</span></span>`}${state.case.hasSecondary?`<span class="kpi">משני: כן</span>`:`<span class="kpi">משני: לא</span>`}
        </div>
      </div>
  ...
  function requiredPathsForPerson(prefix, person){
    const req = ["firstName","lastName","idNumber","phone"];
    return req.map(k=> `${prefix}.${k}`);
  }

  function validateDetailsStep(showErrors=true){
    const missing = [];

    // Primary
    const c = state.case.client;
    if (!c.firstName) missing.push("client.firstName");
    if (!c.lastName) missing.push("client.lastName");
    if (!c.idNumber) missing.push("client.idNumber");
    if (!c.phone) missing.push("client.phone");

    // Secondary (only if enabled)
    if (state.case.hasSecondary){
      const s = state.case.secondary || {};
      if (!s.firstName) missing.push("secondary.firstName");
      if (!s.lastName) missing.push("secondary.lastName");
      if (!s.idNumber) missing.push("secondary.idNumber");
      if (!s.phone) missing.push("secondary.phone");
    }

    // Children (only validate what exists)
    (state.case.children||[]).forEach((ch, idx)=>{
      if (!ch.firstName) missing.push(`children.${idx}.firstName`);
      if (!ch.lastName) missing.push(`children.${idx}.lastName`);
      if (!ch.birthDate) missing.push(`children.${idx}.birthDate`);
    });
    ...
    if (showErrors){
      ...
      if (missing.length){
        markMissing(missing);
        toast("חסרים פרטים חובה. מלא את השדות המסומנים באדום.");
      }
    }
    return missing.length === 0;
  }

  function validateProductsStep(showErrors=true){
    const missing = [];
    (state.case.products||[]).forEach((p, idx)=>{
      // Require only if row has anything or if at least one product exists
      const hasAny = (p.company || p.productName || p.premiumAfter || p.premiumBefore || p.insuredFor);
      if (!hasAny) return;

      if (!p.insuredFor) missing.push(`products.${idx}.insuredFor`);
      if (!p.company) missing.push(`products.${idx}.company`);
      if (!p.productName) missing.push(`products.${idx}.productName`);
      if (!p.premiumAfter) missing.push(`products.${idx}.premiumAfter`);
    });

    if ((state.case.products||[]).length === 0){
      // You can change this later; for now, require at least one product to finish flow
      if (showErrors) toast("חייב להוסיף לפחות מוצר אחד לפני המשך.");
      return false;
    }
    ...
    if (showErrors){
      ...
      if (missing.length){
        markMissing(missing);
        toast("חסרים פרטים במוצרים. מלא את השדות המסומנים באדום.");
      }
    }
    return missing.length === 0;
  }
  ...
  function validateStep(step, showErrors=true){
    if (step === "details") return validateDetailsStep(showErrors);
    if (step === "cancellations"){
      // use existing cancellations validator (allows empty rows but validates partially filled ones)
      const issues = validateCancellationRows();
      if (showErrors && issues.length) toast(issues[0]);
      return issues.length === 0;
    }
    if (step === "products") return validateProductsStep(showErrors);
    // other steps: allow
    return true;
  }

  function getMaxUnlockedStepIndex(){
    const steps = Array.from(stepper.querySelectorAll(".step"));
    const order = steps.map(s=>s.dataset.step);

    // Start at details
    let max = 0;
    // If details valid -> unlock cancellations
    if (validateStep("details", false)) max = Math.max(max, order.indexOf("cancellations"));
    // If cancellations valid too -> unlock products
    if (validateStep("details", false) && validateStep("cancellations", false)) max = Math.max(max, order.indexOf("products"));

    // If current step is before max, still allow moving back freely (handled in click)
    return Math.max(0, max);
  }

  function refreshStepperLocks(){
    const steps = Array.from(stepper.querySelectorAll(".step"));
    const order = steps.map(s=>s.dataset.step);
    const max = getMaxUnlockedStepIndex();
...
function validateCancellationRows(){
  ...
  function renderCancellations(){
    ...
    app.innerHTML = `
      <div class="sectionHead">
        <div class="sectionLeft">
          <div class="sectionTitle">ביטול פוליסות בחברות נגדיות</div>
          <div class="sectionSub">שקף כאן פוליסות קיימות לביטול (מלא/חלקי) לפי כל מבוטח.</div>
        </div>
        <div class="sectionRight">
          ${`<span class="kpi">ראשי: <span class="mono">${(state.case.cancellations?.primary||[]).length}</span></span>`}${state.case.hasSecondary?`<span class="kpi">משני: <span class="mono">${(state.case.cancellations?.secondary||[]).length}</span></span>`:`<span class="kpi">משני: 0</span>`}${`<span class="kpi">סה״כ ילדים: <span class="mono">${(state.case.children||[]).length}</span></span>`}
        </div>
      </div>
...
function renderProductsActions(){
  ...
  function renderProducts(){
    if (state.case.products.length === 0){
      ...
      <div class="sectionHead">
        <div class="sectionLeft">
          <div class="sectionTitle">מוצרים חדשים אצלנו</div>
          <div class="sectionSub">הוסף מוצרים והצמד אותם למבוטח (ראשי/משני/ילד) כדי לקבל סיכום חכם.</div>
        </div>
        <div class="sectionRight">
          ${`<span class="kpi">מוצרים: <span class="mono">${(state.case.products||[]).length}</span></span>`}${`<span class="kpi">סה״כ אחרי: <span class="mono">${String(totalAfter())}</span> ₪</span>`}
        </div>
      </div>
  ...
  function renderSummary(){
        ...
        <div class="card">
          <div class="cardTop">
            <div>
              ...
              <div class="cardMeta">${esc(p.company || "—")} • מבוטח: ${esc(insuredLabelFromCode(p.insuredFor || "primary"))} • ID: <span class="mono">${esc(p.id)}</span></div>
            </div>
    ...
    app.innerHTML = `
      ...
      <div class="card">
        <div class="cardTitle">פוליסות נגדיות לביטול</div>
        <div class="cardMeta">שיקוף מלא של הפוליסות שמסומנות לביטול לפי מבוטח (חברה/פוליסה/פרמיה/מלא-חלקי).</div>
      </div>

      ${renderCancellationsSummary()}
  ...
  function exportCasePDF(c){
    ...
    const cancellationsRows = [];
    const canc = c.cancellations || { primary: [], secondary: [], children: {} };

    (canc.primary || []).forEach((p)=> cancellationsRows.push({ insured:"primary", ...p }));
    if (c.hasSecondary) (canc.secondary || []).forEach((p)=> cancellationsRows.push({ insured:"secondary", ...p }));
    const childMap = canc.children || {};
    (c.children || []).forEach((ch)=> {
      const arr = childMap[ch.id] || [];
      arr.forEach((p)=> cancellationsRows.push({ insured:`child:${ch.id}`, ...p }));
    });

    const cancellationsHtml = cancellationsRows.map((p,i)=>`
      <tr>
        <td>${i+1}</td>
        <td>${esc(insuredLabelFromCode(p.insured))}</td>
        <td>${esc(p.company||"—")}</td>
        <td>${esc(p.product||"—")}</td>
        <td class="mono">${esc(p.premiumPaid||"—")}</td>
        <td>${esc(p.cancelType||"—")}</td>
      </tr>
    `).join("");
...
<body>
  <div class="page">
    ...
    <div class="blk">
      <div class="blkTitle">פוליסות נגדיות לביטול</div>
      <table class="t">
        <thead><tr><th>#</th><th>מבוטח</th><th>חברה</th><th>פוליסה / ביטוח</th><th>פרמיה</th><th>ביטול</th></tr></thead>
        <tbody>
          ${cancellationsRows.length ? cancellationsHtml : `<tr><td colspan="6">אין פוליסות לביטול</td></tr>`}
        </tbody>
      </table>
    </div>
  ...
  function renderSavedActions(){
    ...
    panelActions.querySelector("#btnNewCase").addEventListener("click", ()=>{
      if (confirm("לפתוח תיק חדש? זה לא מוחק לקוחות.")){
  ...
  function renderSaved(){
    // Header
    caseTitle.textContent = "לקוחות";
    caseIdChip.textContent = "GICRM_CASES";
    caseStatusChip.textContent = "מאגר מקומי";
    safeText(stageBadge, "תיקים");
    safeText(badgeStage, "תיקים");

    // hide stepper and keep header
    stepper.classList.add("hidden");

    panelTitle.textContent = "לקוחות";
    ...
    if (list.length === 0){
      app.innerHTML = `
        <div class="card">
          <div class="cardTitle">אין לקוחות</div>
  ...
  function renderSettings(){
    ...
    safeText(stageBadge, "הגדרות");
    safeText(badgeStage, "הגדרות");
...
function safeParse(raw, fallback){
  ...
  stepper.addEventListener("click", (e)=>{
    ...
    // wizard gating: prevent jumping forward if current step invalid
    refreshStepperLocks();
    if (btn.classList.contains("locked")){
      toast("קודם מסיימים את השלב הנוכחי 👈");
      return;
    }
    ...
    const steps = Array.from(stepper.querySelectorAll(".step"));
    const order = steps.map(s=>s.dataset.step);
    const currIdx = order.indexOf(state.case.step);
    const targetIdx = order.indexOf(step);
    if (targetIdx > currIdx){
      const ok = validateStep(state.case.step, true);
      if (!ok) return;
    }
  ...
  function totalAfter(){
    return (state.case.products||[]).reduce((a,p)=>a+num(p.premiumAfter),0);
  }


// Wire Sticky Actions to existing controls (safe)
(function(){
  const back = document.getElementById('stickyBack');
  const save = document.getElementById('stickySave');
  const next = document.getElementById('stickyNext');
  const btnPrev = document.getElementById('btnPrev');
  const btnSave = document.getElementById('btnPrimary') || document.getElementById('btnSave');
  const btnNext = document.getElementById('btnNext');

  if(back && btnPrev) back.addEventListener('click', ()=> btnPrev.click());
  if(save && btnSave) save.addEventListener('click', ()=> btnSave.click());
  if(next && btnNext) next.addEventListener('click', ()=> btnNext.click());
})();



/* === Premium UX Pack wiring === */
(function(){
  ...
  function setSaveStatus(mode, text){
    if(!saveEl) return;
    saveEl.classList.remove('ok','busy','err');
    if(mode) saveEl.classList.add(mode);
    saveEl.textContent = text || 'מוכן';
  }
  // Expose globally for existing save flows to call if desired
  window.__setSaveStatus = setSaveStatus;

  // Try to hook into common save flows without changing business logic
  // If there is a primary save button, mark busy on click and ok after a short delay;
  // if the app exposes events, it will override this.
  const primarySave = document.getElementById('btnPrimary') || document.getElementById('btnSave');
  if(primarySave){
    primarySave.addEventListener('click', ()=>{
      setSaveStatus('busy','שומר…');
      // fallback success indicator (actual save logic may be async)
      setTimeout(()=> setSaveStatus('ok','נשמר'), 900);
      setTimeout(()=> setSaveStatus('', 'מוכן'), 2400);
    });
  }

  // Step rail: map current "stage" to dots if app has stage index
  const dots = $$('.stepDot');
  function setStep(i){
    dots.forEach(d=> d.classList.toggle('active', String(i) === d.getAttribute('data-step')));
  }
  // If app has global stage var, attempt to read it
  try{
    if(typeof window.stageIndex === 'number') setStep(window.stageIndex);
  }catch(_){}

  // Provide a small API for the app to call
  window.__setStep = setStep;

  // Smart title: if there is a current customer name element, mirror it
  const titleEl = document.getElementById('smartCaseTitle');
  const nameEl = document.getElementById('custName') || document.getElementById('customerName');
  if(titleEl && nameEl){
    const syncTitle = ()=> {
      const v = (nameEl.value || nameEl.textContent || '').trim();
      titleEl.textContent = v ? v : 'הקמת לקוח חדש';
    };
    syncTitle();
    nameEl.addEventListener && nameEl.addEventListener('input', syncTitle);
  }

  // Empty states: show only when related containers are empty (best-effort)
  const emptySecondary = document.getElementById('emptySecondary');
  const emptyKids = document.getElementById('emptyKids');
  const emptyPolicies = document.getElementById('emptyPolicies');
  const addSec = document.getElementById('emptyAddSecondary');
  const addKid = document.getElementById('emptyAddKid');

  // Try find existing add buttons in UI
  const btnAddSecondary = document.getElementById('btnAddSecondary') || document.querySelector('[data-action="addSecondary"]');
  const btnAddKid = document.getElementById('btnAddKid') || document.querySelector('[data-action="addKid"]');

  if(addSec && btnAddSecondary){
    addSec.addEventListener('click', ()=> btnAddSecondary.click());
  }
  if(addKid && btnAddKid){
    addKid.addEventListener('click', ()=> btnAddKid.click());
  }

  function isListEmpty(selector){
    const el = document.querySelector(selector);
    if(!el) return null;
    const hasItems = el.querySelectorAll('tr, .row, .item, .pill, .card').length > 0;
    return !hasItems;
  }

  function refreshEmpty(){
    // Best-effort selectors (won't break if missing)
    const secEmpty = isListEmpty('#secondaryList') ?? isListEmpty('#secondarySection') ?? null;
    const kidsEmpty = isListEmpty('#kidsList') ?? isListEmpty('#kidsSection') ?? null;
    const polEmpty = isListEmpty('#policiesList') ?? isListEmpty('#policiesSection') ?? null;

    if(emptySecondary && secEmpty !== null) emptySecondary.hidden = !secEmpty;
    if(emptyKids && kidsEmpty !== null) emptyKids.hidden = !kidsEmpty;
    if(emptyPolicies && polEmpty !== null) emptyPolicies.hidden = !polEmpty;
  }

  // refresh on load and on common events
  refreshEmpty();
  document.addEventListener('click', ()=> setTimeout(refreshEmpty, 50));
})();



// === Fullscreen Clients modal (placeholder) ===
(function(){
  const modal = document.getElementById('clientsModal');
  const closeBtn = document.getElementById('clientsCloseBtn');
  if(!modal) return;
  ...
  modal.addEventListener('click', (e)=>{
    const t = e.target;
    if(t && t.getAttribute && t.getAttribute('data-close') === 'clients') close();
  });
  document.addEventListener('keydown', (e)=>{
    if(e.key === 'Escape' && modal.classList.contains('open')) close();
  });

  // Open when clicking the sidebar "לקוחות"
  document.addEventListener('click', (e)=>{
    const btn = e.target && e.target.closest ? e.target.closest('.navItem[data-nav="saved"]') : null;
    if(btn){
      e.preventDefault();
      e.stopPropagation();
