...
async function gsFetch(url, opts){
  ...
  try{
    ...
    // If Google returns an HTML page (permissions/deploy), show a clear error
    const isHtml = /^\s*</.test(txt) && /<html|<!doctype/i.test(txt);
    let data = null;
    try{ data = txt ? JSON.parse(txt) : null; }catch(e){}
    if(!res.ok){
      throw new Error(
      (data && (data.error||data.message)) ||
      (isHtml ? "השרת החזיר דף HTML (הרשאות/Deploy). ודא שה-Web App פרוס כ: Execute as Me, Access: Anyone" : ("HTTP " + res.status))
    );
    }
    if(isHtml && !data){
      throw new Error("השרת החזיר HTML במקום JSON — בדוק הרשאות/Deploy של ה-Web App");
    }
    return data;
  }finally{
    clearTimeout(t);
...
function openClientDossier(id){
  ...
  // Close
  el("clientDossierClose")?.addEventListener("click", closeClientDossier);

  // Add policy/doc/event
  el("cd_addPolicy")?.addEventListener("click", ()=>{ const lead=_cdGetLead(); if(!lead) return; _cdOpenPolicyEditor(lead, null); });
  ...
  el("clientDossierAddNote")?.addEventListener("click", addEventFromDossier);
  el("cd_addEvent2")?.addEventListener("click", addEventFromDossier);
