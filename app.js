...
function gotoStep(i){
  ...
  function renderCustomerStep(){
    ...
    wizardMain.innerHTML = `
      ...
      ${insuredId==='main' ? `
      <div class="grid3" style="margin-top:10px">
        ${field('כתובת מלאה', 'customer_address', d.address || '')}
        ${field('עיר', 'customer_city', d.city || '')}
        <div class="field">
          <label>מיקוד</label>
          <div class="inputRow">
            <input id="customer_postalCode" placeholder="לדוגמה: 6100000" value="${escapeAttr(d.postalCode || '')}" />
            <button class="btn btnTiny" id="postalLookupBtn" type="button">שלוף</button>
          </div>
          <div class="hintTiny">אופציונלי • אפשר להזין ידנית או ללחוץ שליפה לפי כתובת</div>
        </div>
      </div>` : ''}
    ...
    // Address (main insured only)
    bindInput('customer_address', v => d.address = v);
    bindInput('customer_city', v => d.city = v);
    bindInput('customer_postalCode', v => d.postalCode = v);

    const postalBtn = document.getElementById('postalLookupBtn');
    if(postalBtn){
      postalBtn.addEventListener('click', async () => {
        const addr = String(d.address || '').trim();
        const city = String(d.city || '').trim();
        if(!addr){
          toast('חסר', 'הזן כתובת מלאה כדי לשלוף מיקוד');
          return;
        }
        const query = (addr + (city ? (' ' + city) : '') + ' ישראל').trim();
        postalBtn.disabled = true;
        const oldText = postalBtn.textContent;
        postalBtn.textContent = '...';
        try{
          const pc = await fetchPostalCodeFromAddress_(query);
          if(pc){
            d.postalCode = pc;
            const inp = document.getElementById('customer_postalCode');
            if(inp) inp.value = pc;
            toast('עודכן', 'המיקוד עודכן לפי הכתובת');
          }else{
            toast('לא נמצא', 'לא מצאנו מיקוד לכתובת הזו. אפשר להזין ידנית.');
          }
        }catch(e){
          console.error('postal lookup failed', e);
          toast('שגיאה', 'שליפת מיקוד נכשלה (בדוק חיבור אינטרנט / חסימת CORS)');
        }finally{
          postalBtn.disabled = false;
          postalBtn.textContent = oldText || 'שלוף';
        }
      });
    }
...
function setSelectValue(id, value){
  ...
  // ---------- Address helpers ----------
  async function fetchPostalCodeFromAddress_(q){
    // Best-effort lookup via OpenStreetMap Nominatim.
    const url = 'https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&limit=1&q=' + encodeURIComponent(String(q||'').trim());
    const res = await fetch(url, { headers: { 'Accept':'application/json' } });
    const data = await res.json().catch(() => null);
    if(!Array.isArray(data) || !data.length) return '';
    const a = (data[0] && data[0].address) ? data[0].address : {};
    return String(a.postcode || '').trim();
  }
