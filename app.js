...
const DEFAULT_WEBAPP_URL = "https://script.google.com/macros/s/AKfycbxlUEK0Z9rLM6ng3exKtQdYSHbg0g-MV1TtyornEH3cu3-Q_CP6cTBILV-IdJjMGayUGw/exec";
...
function loadCfg(){
  try{
    ...
    return {
      ...
      webAppUrl: (cfg.webAppUrl || DEFAULT_WEBAPP_URL || "").trim(),
...
function gsJsonp(url){
  return new Promise((resolve, reject)=>{
    const cb = "gs_cb_" + Math.random().toString(16).slice(2);
    let script = null;
    const cleanup = ()=>{
...
async function gsFetch(url, opts){
  ...
  // GitHub Pages cross-origin: Apps Script does not reliably send CORS headers.
  // לכן GET מתבצע דרך JSONP (אם ה-WebApp תומך callback).
  if(method === "GET"){
    try{
      return await gsJsonp(url);
    }catch(e){
      throw new Error("קריאת GET נחסמה (CORS). פרסם מחדש את Apps Script עם תמיכת JSONP (callback).");
    }
  }

  // POST: fire-and-forget (no-cors) כדי להימנע מ-preflight/CORS.
  // נשלח כ-text/plain כדי להישאר בקשה 'פשוטה'.
  const body = (opts && opts.body) ? opts.body : "";

  // Prefer sendBeacon when available (more reliable cross-origin).
  try{
  ...
  try{
    ...
    throw new Error("שליחת POST נחסמה/נכשלה. בדוק פריסה (Anyone) וכתובת /exec.");
  }
}
...
async function gsPush(){
  ...
  const data = await gsFetch(url, { method:"POST", body });
  appCfg = saveCfg({ lastOkAt: nowISO() });
  return data;
}
...
/* ===== Login Gate Logic (restored) =====
   ...
   - Default password: 1234  (change DEFAULT_PASSWORD below if you want)
   - Remembers login per-tab (sessionStorage) so refresh doesn't ask again.
*/
(function(){
  const DEFAULT_PASSWORD = "1234";            // <-- שנה כאן אם תרצה סיסמה אחרת
