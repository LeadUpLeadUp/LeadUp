...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  function upsertUser_(payload){
    ...
    // uniqueness
    const exists = users.find(u => String(u.username||'').trim().toLowerCase() === username && (!isEdit || String(u.username||'').trim().toLowerCase() !== prevUsername));
    if(exists) return { ok:false, msg:'שם משתמש כבר קיים' };

    // prevent demoting/disable last admin
    if(isEdit){
      const cur = users.find(u => String(u.username||'').trim().toLowerCase() === prevUsername);
    ...
    if(!isEdit){
      ...
      const idx = users.findIndex(u => String(u.username||'').trim().toLowerCase() === prevUsername);
      ...
      if(state.currentUser && String(state.currentUser.username||'').trim().toLowerCase() === prevUsername){
  ...
  function setUserActive_(username, active){
    const u = (String(username||'').trim().split(/\s+/)[0] || '').toLowerCase();
    const users = loadUsers_();
    const idx = users.findIndex(u2 => String(u2.username||'').trim().toLowerCase() === u);
    if(idx < 0) return { ok:false, msg:'משתמש לא נמצא' };
  ...
  function deleteUser_(username){
    ...
    if(state.currentUser && String(state.currentUser.username||'').trim().toLowerCase() === u){
  ...
  function findUserByUsername_(username){
    ...
    // - trim whitespace
    // - take first token (in case user pasted "admin ..." by mistake)
    // - case-insensitive match
    const raw = String(username||'').trim();
    const u = (raw.split(/\s+/)[0] || '').toLowerCase();
