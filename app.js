...
function renderInsuredComparisonSummary(){
  ...
  function render(){
    ...
    if (state.view === "saved") { document.body.classList.remove("detailsFull"); gridEl?.classList.remove("wide"); renderSaved(); return; }
    if (state.view === "settings") { document.body.classList.remove("detailsFull"); gridEl?.classList.remove("wide"); renderSettings(); return; }
    ...
    // Full-screen workspace on Details (so it feels like a real system, not a form)
    document.body.classList.toggle("detailsFull", state.case.step === "details");
    gridEl?.classList.toggle("wide", state.case.step === "details");
    shellEl?.classList.toggle("wide", state.case.step === "details");
  ...
  function validatePrimarySubsection(){
    const c = state.case.client || {};
    return !!(c.firstName && c.lastName && c.idNumber && c.phone);
  }
  function validateSecondarySubsection(){
    if (!state.case.hasSecondary) return true;
    const s = state.case.secondary || {};
    return !!(s.firstName && s.lastName && s.idNumber && s.phone);
  }
  function validateChildrenSubsection(){
    const kids = state.case.children || [];
    if (!kids.length) return true;
    return kids.every(ch => ch.firstName && ch.lastName && ch.birthDate);
  }
;
