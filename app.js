...
// ---------- UI Density (Compact mode for desktop app / PWA) ----------
const DENSITY_LS_KEY = 'gemel_density_v1';

function isStandalone_(){
  // Chrome PWA / installed app
  try{
...
function applyDensity_(mode){
  const m = (mode === 'compact') ? 'compact' : 'comfortable';
  if(!state.ui) state.ui = { density: m };
  state.ui.density = m;

  document.documentElement.classList.toggle('density-compact', m === 'compact');
  document.body.classList.toggle('density-compact', m === 'compact');
}

function setDensity_(mode){
  const m = (mode === 'compact') ? 'compact' : 'comfortable';
  try{ localStorage.setItem(DENSITY_LS_KEY, m); }catch(_){}
  applyDensity_(m);
}

function initDensity_(){
  const saved = getDensity_();
  if(saved){
    applyDensity_(saved);
    return;
  }
  // Default: if installed app → compact, otherwise comfortable
  applyDensity_(isStandalone_() ? 'compact' : 'comfortable');
}
...
<div class="sideCard">
  <div class="sideTitle">תצוגה לנציגים</div>
  <label class="prefRow">
    <span>תצוגה קומפקטית (מומלץ לאפליקציה)</span>
    <input id="densityToggle" type="checkbox" ${state.ui?.density==='compact'?'checked':''}/>
  </label>
  <div class="sideHint">מקטין מעט את כל המערכת כדי שירגיש טבעי ונוח ב־Desktop App.</div>
</div>
    ...
    if(densityToggle){
      densityToggle.addEventListener('change', () => {
        setDensity_(densityToggle.checked ? 'compact' : 'comfortable');
      });
    }
