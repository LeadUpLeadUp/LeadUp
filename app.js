...
function wireGsUI(){
  // Use delegated listeners so it keeps working even if the Settings view is re-rendered (innerHTML).
  if(window.__LEADUP_GS_UI_BOUND) { 
    // still refresh labels/values if elements exist
    try{ updateGsUI(); }catch(e){}
    return; 
  }
  window.__LEADUP_GS_UI_BOUND = true;

  // Helpers to read current inputs (they may be recreated)
  const readCfgFromUI = ()=>{
    const urlInp = el("gsUrl");
    const auto = el("gsAuto");
    return {
      webAppUrl: normalizeWebAppUrl(urlInp ? urlInp.value : ""),
      autoSync: !!(auto && auto.checked)
    };
  };

  // Persist URL/autosync when user changes them
  document.addEventListener("change", ()=>{
    try{
      const {webAppUrl, autoSync} = readCfgFromUI();
      appCfg = saveCfg({ webAppUrl, autoSync });
      updateGsUI();
    }catch(e){}
  }, true);
  ...
  document.addEventListener("click", async (e)=>{
    ...
    if(t.closest && t.closest("#gsTest")){
      ...
      try{
        ...
        toast("חיבור ל־Google Sheets תקין ✅", "success");
      }catch(err){
        console.error("gsTest failed", err);
        gsSetStatus("בדיקת חיבור נכשלה ❌", false);
        toast("בדיקת חיבור נכשלה: " + (err?.message || err), "danger");
      }
      return;
    }

    // Pull
    if(t.closest && t.closest("#gsPull")){
      e.preventDefault();
      try{
        const {webAppUrl, autoSync} = readCfgFromUI();
        appCfg = saveCfg({ webAppUrl, autoSync });
        updateGsUI();
        gsSetStatus("טוען נתונים מהשיט...", true);
        await gsPull();
        gsSetStatus("נטען מהשיט ✅", true);
        toast("נטען מהשיט ✅", "success");
        renderAll();
      }catch(err){
        console.error("gsPull failed", err);
        gsSetStatus("טעינה מהשיט נכשלה ❌", false);
        toast("טעינה מהשיט נכשלה: " + (err?.message || err), "danger");
      }
      return;
    }

    // Push
    if(t.closest && t.closest("#gsPush")){
      e.preventDefault();
      try{
        const {webAppUrl, autoSync} = readCfgFromUI();
        appCfg = saveCfg({ webAppUrl, autoSync });
        ...
        toast("נשמר לשיט ✅", "success");
      }catch(err){
        console.error("gsPush failed", err);
        gsSetStatus("שמירה לשיט נכשלה ❌", false);
        toast("שמירה לשיט נכשלה: " + (err?.message || err), "danger");
      }
      return;
    }

    // Mode toggle
    if(t.closest && t.closest("#gsModeBtn")){
      e.preventDefault();
      try{
        const {webAppUrl, autoSync} = readCfgFromUI();
        const nextMode = (appCfg.mode === "local") ? "sheets" : "local";

        // Always save URL/autosync first
        appCfg = saveCfg({ webAppUrl, autoSync, mode: nextMode });
        updateGsUI();

        if(nextMode === "sheets"){
          if(!webAppUrl){
            gsSetStatus("חסר URL של Web App", false);
            toast("הדבק את ה־URL של ה־Web App ואז נסה שוב", "warn");
            // rollback
            appCfg = saveCfg({ mode:"local" });
            updateGsUI();
            return;
          }
          gsSetStatus("מתחבר…", true);
          await gsPing();         // verify
          await gsPull();         // initial load
          appCfg = saveCfg({ lastOkAt: Date.now() });
          updateGsUI();
          gsSetStatus("מחובר ✅", true);
          toast("עברנו למצב Google Sheets ✅", "success");
          renderAll();
        }else{
          gsSetStatus("מצב מקומי (LocalStorage)", true);
          toast("חזרה למצב Local ✅", "success");
        }
      }catch(err){
        console.error("mode switch failed", err);
        gsSetStatus("חיבור נכשל (בדוק URL/הרשאות)", false);
        toast("לא הצלחתי להתחבר לשיט: " + (err?.message || err), "danger");
        // rollback
        try{ appCfg = saveCfg({ mode:"local" }); updateGsUI(); }catch(e){}
      }
      return;
    }
  }, true);
