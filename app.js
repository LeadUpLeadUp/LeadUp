...
`;
  ...
  function renderSummaryStep(){
    ...
    // Bind agent fields
    bindInput('agent_number', v => p.agentInfo.number = String(v||'').trim());
    bindSelect('agent_company', v => p.agentInfo.company = String(v||'').trim());
  ...
  function openPrintView(proposal){
    // Some browsers/PWA standalone block window.open or window.print.
    // We'll try a popup first, and if it fails, fall back to an offscreen iframe print.
    ...
    try {
      ...
      const tryPrint = (win) => {
        try{
          win.focus();
          // Delay slightly so layout/fonts/images are ready before opening the print dialog.
          setTimeout(() => { try{ win.print(); }catch(_){} }, 200);
        }catch(_){}
      };

      // Popup blocked? Fall back to iframe print (works in many PWA standalone cases).
      if(!w){
        try{
          ...
          console.error(e);
          toast('שגיאה', 'הדפדפן חסם יצוא PDF');
        }
        return;
      }
