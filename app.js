...
function renderNewPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="tableWrap">
        <table>
          <thead><tr><th>חברה</th><th>מוצר</th><th>פרמיה</th><th>הנחות</th><th>אחרי הנחה</th><th>כיסויים</th><th>שיעבוד</th><th></th></tr></thead>
          <tbody>
            ${rows.length ? rows.map(({x,idx}) => `
              <tr>
                <td>${escapeHtml(x.company)}</td>
...
`;
  ...
  function openPrintView(proposal){
    ...
    // ---- Build insured list (only insured with any data) ----
    const insuredList = (p.insuredList||[]).filter(ins => insuredHasAnyData(p, ins.id));
    const insuredLabel = (id) => labelInsured(id);

    // ---- Totals (premiums / discounts) ----
    const newPolicies = (p.newPolicies||[]);
    const oldPolicies = (p.oldPolicies||[]);

    const sum = (arr, fn) => arr.reduce((a,x)=>a+(Number(fn(x))||0),0);

    const totalNewBefore = sum(newPolicies, x => Number(x && x.premium) || 0);
    const totalNewAfter  = sum(newPolicies, x => premiumAfterDiscount(x));
    const totalNewDiscount = Math.max(0, totalNewBefore - totalNewAfter);

    const totalOld = sum(oldPolicies, x => Number(x && x.premium) || 0);

    const customerName = (p.customer && (p.customer.fullName||'')) || p.customerName || '';
    const createdAtStr = p.createdAt ? new Date(p.createdAt).toLocaleString('he-IL') : '—';

    // ---- Customer summary table (compact) ----
    const customerSummaryHtml = `
      <table class="kv">
        <tbody>
          <tr>
            <td><b>לקוח</b></td><td>${esc(customerName)}</td>
            <td><b>ת״ז</b></td><td>${esc(p.customer && p.customer.tz)}</td>
            <td><b>נייד</b></td><td>${esc(p.customer && (p.customer.phone || p.customer.mobile))}</td>
            <td><b>אימייל</b></td><td>${esc(p.customer && p.customer.email)}</td>
          </tr>
          <tr>
            <td><b>נציג</b></td><td>${esc(p.assignedAgent || p.agentName || '')}</td>
            <td><b>סטטוס</b></td><td>${esc(p.status)}</td>
            <td><b>סטטוס פנימי</b></td><td>${esc(p.internalStatus)}</td>
            <td><b>נוצר</b></td><td colspan="3">${esc(createdAtStr)}</td>
          </tr>
        </tbody>
      </table>
    `;

    // ---- Insured details table (one table; one row per insured) ----
    function calcBMI(hCm, wKg){
      ...
      const m = h / 100;
      const bmi = w / (m*m);
      return isFinite(bmi) ? (Math.round(bmi*10)/10).toFixed(1) : '';
    }
    ...
    const insuredRows = insuredList.map(ins => {
      ...
      return `
        <tr>
          <td>${esc(d.label || insuredLabel(ins.id))}</td>
          ...
          <td>${esc(d.hmo)}</td>
          <td>${esc(d.shaban)}</td>
          <td>${d.heightCm ? esc(d.heightCm + ' ס״מ') : '—'}</td>
          <td>${d.weightKg ? esc(d.weightKg + ' ק״ג') : '—'}</td>
          <td>${bmi ? esc(bmi) : '—'}</td>
    ...
    const insuredTableHtml = `
      <table>
        <thead>
          <tr>
            <th style="width:110px">מבוטח</th>
            <th>שם</th>
            <th style="width:90px">ת״ז</th>
            <th style="width:110px">נייד</th>
            <th>אימייל</th>
            <th style="width:95px">ת. לידה</th>
            <th style="width:65px">מין</th>
            <th style="width:110px">קופ״ח</th>
            <th style="width:80px">שב״ן</th>
            <th style="width:70px">גובה</th>
            <th style="width:70px">משקל</th>
            <th style="width:55px">BMI</th>
            <th>עיסוק</th>
          </tr>
        </thead>
        <tbody>
          ${insuredRows || `<tr><td colspan="13" class="muted">אין מבוטחים</td></tr>`}
        </tbody>
      </table>
    `;

    // ---- Policies tables (single table with insured column) ----
    function buildOldPoliciesTable(){
      if(!oldPolicies.length) return `<div class="muted">אין פוליסות לביטול.</div>`;
      const rows = oldPolicies.map(x => `
        <tr>
          <td>${esc(insuredLabel(x.insuredId || 'main'))}</td>
          <td>${esc(x.company)}</td>
          <td>${esc(x.product)}</td>
          <td>${money(x.premium||0)}</td>
          <td>${esc(x.policyNumber || x.policyNo || '')}</td>
          <td>${esc(x.notes || '')}</td>
        </tr>
      `).join('');
      return `
        <table>
          <thead>
            <tr>
              <th style="width:110px">מבוטח</th>
              <th style="width:150px">חברה</th>
              <th>מוצר</th>
              <th style="width:110px">פרמיה</th>
              <th style="width:130px">מס׳ פוליסה</th>
              <th>הערות</th>
            </tr>
          </thead>
          <tbody>${rows}</tbody>
          <tfoot>
            <tr>
              <td colspan="3"><b>סה״כ פרמיה קיימת</b></td>
              <td colspan="3"><b>${money(totalOld)}</b></td>
            </tr>
          </tfoot>
        </table>
      `;
    ...
    function buildNewPoliciesTable(){
      if(!newPolicies.length) return `<div class="muted">אין פוליסות חדשות.</div>`;
      const rows = newPolicies.map(x => {
        const discTier = (x.discount && x.discount.tier) ? String(x.discount.tier).trim() : '';
        const discText = summarizeDiscount(x.discount);
        const after = premiumAfterDiscount(x);
        const cov = x.coveragesText ? esc(x.coveragesText) : '—';
        const lien = x.lien ? esc([x.lien.bankName, x.lien.branch, x.lien.years ? (x.lien.years + ' שנים') : ''].filter(Boolean).join(' • ')) : '—';
        return `
          <tr>
            <td>${esc(insuredLabel(x.insuredId || 'main'))}</td>
            <td>${esc(x.company)}</td>
            <td>${esc(x.product)}</td>
            <td>${money(x.premium||0)}</td>
            <td>${discTier && discTier!=='0%' ? esc(discTier) : '—'}</td>
            <td>${discText ? esc(discText) : '—'}</td>
            <td>${money(after)}</td>
            <td>${cov}</td>
            <td>${lien}</td>
          </tr>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style="width:150px">חברה</th>
              <th>מוצר</th>
              <th style="width:110px">פרמיה (לפני)</th>
              <th style="width:90px">מדורג</th>
              <th>הנחות / הערה</th>
              <th style="width:110px">פרמיה (אחרי)</th>
              <th>כיסויים</th>
              <th style="width:170px">שיעבוד</th>
            </tr>
          </thead>
          <tbody>${rows}</tbody>
          <tfoot>
            <tr>
              ...
              <td><b>${money(totalNewBefore)}</b></td>
              <td colspan="2"><b>סה״כ הנחה: ${money(totalNewDiscount)}</b></td>
              <td><b>${money(totalNewAfter)}</b></td>
              <td colspan="2"></td>
            </tr>
          </tfoot>
        </table>
    ...
    function oldSwitchReasonsHtml(){
      if(Array.isArray(p.oldPolicySwitchReasons) && p.oldPolicySwitchReasons.length){
        const items = p.oldPolicySwitchReasons.map(k => {
          const r = OLD_POLICY_SWITCH_REASONS.find(x => x.key === k);
          return '<li>' + esc(r ? r.label : k) + '</li>';
        }).join('');
        return `<ul class="reasons">${items}</ul>`;
      }
      return `<div class="muted">לא נבחרו סיבות.</div>`;
    }

    // ---- Payment block (keep compact; internal use) ----
      ...
      if(method==='card'){
        ...
        return `
          <table class="kv">
            <tbody>
              <tr>
                <td><b>אמצעי</b></td><td>כרטיס אשראי</td>
                <td><b>ת״ז בעל הכרטיס</b></td><td>${esc(c.tz)}</td>
                <td><b>שם בעל הכרטיס</b></td><td>${esc(c.name)}</td>
                <td><b>4 ספרות</b></td><td>${c.last4 ? esc('**** ' + c.last4) : '—'}</td>
              </tr>
              <tr>
                <td><b>תוקף</b></td><td>${esc(c.exp)}</td>
                <td><b>הערות</b></td><td colspan="5">${esc(pay.notes || '')}</td>
              </tr>
            </tbody>
          </table>
        `;
      ...
      if(method==='hok'){
        ...
        return `
          <table class="kv">
            <tbody>
              <tr>
                <td><b>אמצעי</b></td><td>הוראת קבע (הו״ק)</td>
                <td><b>בנק</b></td><td>${esc(h.bank)}</td>
                <td><b>סניף</b></td><td>${esc(h.branch)}</td>
                <td><b>חשבון</b></td><td>${esc(h.account)}</td>
              </tr>
              <tr>
                <td><b>תוקף</b></td><td>${esc(h.exp)}</td>
                <td><b>הערות</b></td><td colspan="5">${esc(pay.notes || '')}</td>
              </tr>
            </tbody>
          </table>
    ...
    // ---- Medical tables (per insured; two-column) ----
    function medicalTables(){
      ...
      for(const ins of insuredList){
        ...
        out.push(`
          <div class="block">
            <div class="btitle">שאלון רפואי – ${esc(insuredLabel(ins.id))}</div>
            <table>
              <thead><tr><th>שאלה</th><th>תשובה</th></tr></thead>
              <tbody>${rows.length ? rows.join('') : `<tr><td colspan="2">אין תשובות</td></tr>`}</tbody>
            </table>
    ...
    // ---- Optional: dump of extra stored fields (helps ensure "כל נתון אפשרי") ----
    function extraFieldsTable(){
      // Keep only safe, internal fields; skip bulky collections and obvious UI-only keys.
      const SKIP = new Set([
        'oldPolicies','newPolicies','insuredList','medical','payer','customer',
        'pdfGenerated','createdAt','id','customerId','customerName','assignedAgent'
      ]);
      ...
      for(const [k,v] of Object.entries(p||{})){
        ...
        const t = typeof v;
        if(t==='object') continue; // avoid nested blobs
        const s = String(v).trim();
        if(!s) continue;
        pairs.push([k, s]);
      }

      if(!pairs.length) return `<div class="muted">אין.</div>`;

      return `
        <table>
          <thead><tr><th style="width:220px">שדה</th><th>ערך</th></tr></thead>
          <tbody>
            ${pairs.map(([k,v]) => `<tr><td>${esc(k)}</td><td>${esc(v)}</td></tr>`).join('')}
          </tbody>
        </table>
      `;
    }
    ...
    const html = `
      ...
      <head>
        <meta charset="utf-8"/>
        <title>דו״ח תפעולי - ${escapeHtml(customerName)}</title>
        <style>
          @page{ size:A4 landscape; margin:10mm; }
          body{font-family:Arial, sans-serif; margin:0; color:#121319; -webkit-print-color-adjust:exact; print-color-adjust:exact;}
          .wrap{padding:10mm;}
          ...
          th,td{border:1px solid #dcdcdc; padding:6px 7px; text-align:right; font-size:11.5px; vertical-align:top}
          th{background:#f3f3f3; font-weight:700}
          tfoot td{background:#fafafa}
          .kv td{border:1px solid #e3e3e3}
          .block{margin-top:10px; break-inside:avoid}
          .btitle{font-weight:700; margin:6px 0 6px; font-size:12px}
          .reasons{margin:6px 0 0; padding-right:18px}
          .topbar{display:flex; align-items:flex-end; justify-content:space-between; gap:12px}
          .badge{font-size:11px; padding:3px 8px; border:1px solid #d8d8d8; border-radius:999px; background:#fafafa; color:#333}
          @media print{
            .wrap{padding:0}
          }
        </style>
      ...
      <body>
        <div class="wrap">
          <div class="topbar">
            <div>
              <h1>דו״ח תפעולי – סיכום שיחה</h1>
              <div class="sub">GEMEL INVEST • ${esc(createdAtStr)}</div>
            </div>
            <div class="badge">פנימי • לנציג תפעול</div>
          </div>
          ...
          ${buildOldPoliciesTable()}

          <h2>סיבת החלפה פוליסה בתיק הביטוחי</h2>
          ${oldSwitchReasonsHtml()}

          <h2>פוליסות חדשות שנרכשו (כולל הנחות)</h2>
          ${buildNewPoliciesTable()}
          ...
          <h2>נתונים נוספים (מה שנשמר בתיק)</h2>
          ${extraFieldsTable()}

          <div class="muted" style="margin-top:10px">הורדה כ־PDF: הדפס → Save as PDF</div>
        </div>
    ...
    w.document.close();
      ...
      toast(
        'שגיאה',
        'לא הצלחנו ליצור את קובץ הסיכום. בדוק קונסול (F12) לפרטים.'
      );
    }
  }
...
function openPolicyLienModal(current, onSave){
    ...
    <div style="width:min(620px,95vw);background:rgba(255,255,255,0.98);border-radius:18px;box-shadow:0 22px 70px rgba(0,0,0,0.28);padding:14px;direction:rtl;font-family:inherit">
      ...
      <div class="grid2">
        <div class="field">
          ...
          <input id="l_bank" value="${esc(data.bankName||'')}" placeholder="לדוגמה: לאומי" />
        </div>
        <div class="field">
          <label>סניף</label>
          <input id="l_branch" value="${esc(data.branch||'')}" placeholder="מספר/שם סניף" />
        </div>
        <div class="field" style="grid-column:1/-1">
          <label>כתובת סניף הבנק המשעבד</label>
          <input id="l_addr" value="${esc(data.branchAddress||'')}" placeholder="כתובת מלאה" />
        </div>
        <div class="field">
          <label>לכמה שנים</label>
          <input id="l_years" type="number" min="0" step="1" value="${esc(data.years||'')}" placeholder="למשל 20" />
        </div>
      </div>
  ...
  overlay.querySelector('[data-save]').addEventListener('click', () => {
    const out = {
      bankName: (overlay.querySelector('#l_bank').value || '').trim(),
      branchAddress: (overlay.querySelector('#l_addr').value || '').trim(),
      branch: (overlay.querySelector('#l_branch').value || '').trim(),
      years: (overlay.querySelector('#l_years').value || '').trim(),
    };
    // normalize empty
    const hasAny = Object.values(out).some(v => String(v||'').trim());
    close();
