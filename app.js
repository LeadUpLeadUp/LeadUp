...
function debounce(fn, wait){
  ...
  function computeJofiStats_(agentName){
    ...
    // Some proposals in the wild use different timestamp fields. Normalize once.
    function createdIso_(p){
      if(!p) return '';
      const v = p.createdAt || p.created_at || p.createdISO || p.created || p.ts || p.timestamp || p.date;
      if(typeof v === 'number') return new Date(v).toISOString();
      if(typeof v === 'string' && v.trim()) return v.trim();
      return '';
    }
    function newPolicies_(p){
      if(!p) return [];
      return p.newPolicies || p.policiesNew || p.policies_new || p.policies || p.items || [];
    }

    const rel = (state.proposals||[]).filter(p => String(p.assignedAgent||'') === String(agentName||''));

    const relMonth = rel.filter(p => monthKey_(createdIso_(p)) === mk);
    const relDay   = rel.filter(p => dateKey_(createdIso_(p)) === dk);

    const soldMonth = relMonth.reduce((acc,p)=> acc + sumPremium_(newPolicies_(p),'premium') + sumPremium_(newPolicies_(p),'new_premium'), 0);
    const soldDay   = relDay.reduce((acc,p)=> acc + sumPremium_(newPolicies_(p),'premium') + sumPremium_(newPolicies_(p),'new_premium'), 0);

    // NOTE: cancelledMonth remains available for later, but we may hide it in UI.
  ...
  function renderKpisJofi(){
    ...
    if(isAdmin_()){
      ...
      $('#jofiSaveCfg')?.addEventListener('click', () => {
        ...
        saveJofiCfg_({ monthlyTarget:m, dailyTarget:d });
