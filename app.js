...
function openUserModal_(opt){
  ...
  function renderCustomersTable(rows){
          ...
          <tbody>
            ${rows.map(c => `
              <tr>
                ...
                <td style="text-align:left;white-space:nowrap">
                      <button class="btn btnSoft" data-open-proposal="${c.id}">ערוך הצעה</button>
    ...
    $$('[data-open-proposal]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const lp = latestProposalForCustomer(cid);
        if(lp){
          openWizard({ customerId: cid, proposalId: lp.id, mode: 'edit' });
        }else{
          openWizard({ customerId: cid, mode: 'new' });
        }
      });
    });
  ...
  if(saveEditBtn){
    saveEditBtn.addEventListener('click', () => {
      ...
      if(wizMode !== 'edit'){
        toast('שגיאה', 'מצב עריכה לא פעיל');
        return;
      }
      openEditConfirmModal_(wiz.proposal);
    });
  }

  
  function openEditConfirmModal_(proposal){
    // Modal for edit note + confirmation checkbox
    const overlay = document.createElement('div');
    overlay.style.position = 'fixed';
    overlay.style.inset = '0';
    overlay.style.background = 'rgba(0,0,0,.35)';
    ...
    overlay.style.justifyContent = 'center';
    overlay.innerHTML = `
      <div style="width:min(560px,92vw);background:#fff;border-radius:18px;box-shadow:0 20px 60px rgba(0,0,0,.25);padding:16px 16px 14px;direction:rtl;font-family:inherit">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px">
          <div style="font-weight:900;font-size:16px">שמור עריכת הצעה</div>
          <button class="btn btnSoft" data-x>סגור</button>
        </div>

        <div style="font-size:13px;color:rgba(18,19,25,.65);margin-bottom:6px">אנא לרשום מה בוצע/שונה בהצעה <span style="color:#c33">*</span></div>
        <textarea id="editNote" class="input" style="width:100%;min-height:92px;resize:vertical" placeholder="לדוגמה: עדכנתי פרמיה, הוספתי כיסוי, שיניתי חברה..."></textarea>
        <div id="editErr" style="display:none;margin-top:8px;color:#b00020;font-size:12px"></div>

        <label style="display:flex;gap:10px;align-items:flex-start;margin-top:12px">
          <input id="editAck" type="checkbox" style="margin-top:3px">
          <span style="font-size:13px;color:rgba(18,19,25,.8)">האם אתה מאשר את התיקונים שערכת בהצעה?</span>
        </label>

        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:14px">
          <button class="btn btnSoft" data-cancel>ביטול</button>
          <button class="btn btnPrimary" data-confirm>מאשר</button>
        </div>
      </div>
    `;
    document.body.appendChild(overlay);
    ...
    const errEl = overlay.querySelector('#editErr');
    const noteEl = overlay.querySelector('#editNote');
    const ackEl  = overlay.querySelector('#editAck');
    ...
    overlay.querySelector('[data-confirm]')?.addEventListener('click', () => {
      ...
      if(!note){
        showErr('חובה לרשום מה שונה בהצעה.');
        noteEl?.focus();
        return;
      }
      if(!ack){
        showErr('חובה לאשר את התיקונים שבוצעו.');
        return;
      }

      proposal.editHistory = Array.isArray(proposal.editHistory) ? proposal.editHistory : [];
      proposal.editHistory.unshift({
        at: new Date().toISOString(),
        by: state.currentUser ? (state.currentUser.displayName || state.currentUser.username) : (state.agentName || 'אורח'),
        note
      });
      proposal.updatedAt = new Date().toISOString();
      proposal._lastEditNote = note;

      toast('נשמר', 'העריכה נשמרה בהצלחה');
      close();
      closeWizard();
      render();
    });
...
function openWizard({ customerId=null, proposalId=null, mode=null }){
    const now = new Date();
    let proposal = null;
    ...
    // Wizard mode (new/edit)
    wizMode = (mode || (proposalId ? 'edit' : 'new'));
    wizProposal = proposal;
    if(saveEditBtn){
      saveEditBtn.style.display = (wizMode === 'edit') ? '' : 'none';
    }
    ...
    $('#wizardTitle').textContent = proposal.customerName ? `${wizMode==='edit'?'עריכת הצעה':'הצעה'} – ${proposal.customerName}` : (wizMode==='edit'?'עריכת הצעה':'הצעה חדשה');
    const _ws = $('#wizardSub'); if(_ws) _ws.textContent = (wizMode==='edit' ? 'מצב עריכה — בסיום לחץ "שמור עריכת הצעה"' : '');
    // Status dropdown
    if(statusSelect){
