...
(() => {
  ...
  const BUILD = "20260412-vToolsWorkspaceFix2";
  ...
  const TOOLS_WORKSPACE_URL = "https://leadupleadup.github.io/LeadUp/";
...
UsersGateUI.init();
    ...
    getToolsWorkspaceUrl(){
      try {
        const saved = safeTrim(localStorage.getItem(TOOLS_WORKSPACE_URL_STORAGE_KEY));
        if(saved && /^https?:\/\//i.test(saved)) return saved;
      } catch(_e) {}
      return TOOLS_WORKSPACE_URL;
    },

    renderDiscountSpecView(options = {}){
      const frame = document.getElementById('discountSpecFrame');
      const openNew = document.getElementById('btnDiscountSpecOpenNew');
      const reloadBtn = document.getElementById('btnDiscountSpecReload');
      if(!frame) return;

      const url = this.getToolsWorkspaceUrl();
      if(openNew) openNew.href = url;

      const currentSrc = safeTrim(frame.getAttribute('src'));
      if(currentSrc !== url || options.forceReload){
        frame.src = url;
      }
      ...
      if(reloadBtn && !reloadBtn.dataset.bound){
        ...
        on(reloadBtn, 'click', () => {
          const activeUrl = this.getToolsWorkspaceUrl();
          if(openNew) openNew.href = activeUrl;
          frame.src = activeUrl;
        });
      }
