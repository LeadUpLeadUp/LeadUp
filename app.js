...
async function gsFetch(url, opts){
  ...
  try{
    ...
    if(data===null && txt && txt.trim().startsWith("<")){
      // Likely an HTML page (authorization / error) instead of JSON
      throw new Error("השרת החזיר HTML במקום JSON (בדוק הרשאות/Deploy)");
    }
    if(!res.ok){
