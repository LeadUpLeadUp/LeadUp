...
function renderKpisSettings(){
  ...
  function openWizard({ customerId=null, proposalId=null }){
    ...
    // Defensive: never let a UI helper crash the wizard rendering
    try{ buildInsuredTabs(); }catch(e){ console.error('buildInsuredTabs failed', e); }
    try{ updateInsuredIndicator(); }catch(e){ console.error('updateInsuredIndicator failed', e); }
    try{ renderStepper(); }catch(e){ console.error('renderStepper failed', e); }
    try{ renderWizard(); }catch(e){ console.error('renderWizard failed', e); }
    try{ updateSums(); }catch(e){ console.error('updateSums failed', e); }
...
`;
  ...
  function openPrintView(proposal){
    ...
    try {
      ...
      const fmtMoney = (v) => money(num(v)); // uses existing money()
      const fmtDateTime = (ms) => {
        try { return ms ? new Date(ms).toLocaleString('he-IL') : '—'; } catch(_){ return '—'; }
      };

      // ---------- compact customer ----------
      const customerName = (p.customer && (p.customer.fullName||'')) || p.customerName || '';
      const customerTz   = (p.customer && p.customer.tz) || '';
      const customerPhone= (p.customer && (p.customer.phone || p.customer.mobile)) || '';
      const customerEmail= (p.customer && p.customer.email) || '';

      const agentName = p.assignedAgent || p.agentName || '';
      const createdAt = fmtDateTime(p.createdAt);
      ...
      const insuredRows = insuredList.map(ins => {
        ...
        `;
      }).join('') || `<tr><td colspan="13" class="muted">אין מבוטחים עם נתונים.</td></tr>`;

      // ---------- policies ----------
      const oldPolicies = (p.oldPolicies || []);
      const newPolicies = (p.newPolicies || []);

      const totalOld = oldPolicies.reduce((a,x)=>a+num(x && x.premium),0);
      const totalNewBefore = newPolicies.reduce((a,x)=>a+num(x && x.premium),0);
      const totalNewAfter  = newPolicies.reduce((a,x)=>a+num(premiumAfterDiscount(x)),0);
      const totalNewDiscount = Math.max(0, totalNewBefore - totalNewAfter);

      const oldRows = (oldPolicies.length ? oldPolicies : []).map(x => `
        <tr>
          <td>${esc(labelInsured(x.insuredId || 'main'))}</td>
          <td>${esc(x.company)}</td>
          <td>${esc(x.product)}</td>
          <td class="num">${fmtMoney(x.premium)}</td>
        </tr>
      `).join('') || `<tr><td colspan="4" class="muted">אין פוליסות קיימות.</td></tr>`;

      const discountText = (x) => {
        const d = x && x.discount;
        if(!d) return '—';
        const parts = [];
      ...
      const newRows = (newPolicies.length ? newPolicies : []).map(x => {
        const before = num(x && x.premium);
        const after  = num(premiumAfterDiscount(x));
        ...
        return `
          <tr>
            ...
            <td>${esc(x.company)}</td>
            <td>${esc(x.product)}</td>
            <td class="num">${fmtMoney(before)}</td>
            <td class="num">${fmtMoney(after)}</td>
            <td class="num">${fmtMoney(disc)}</td>
            <td>${discountText(x)}</td>
          </tr>
        `;
      }).join('') || `<tr><td colspan="7" class="muted">אין פוליסות חדשות.</td></tr>`;

      // ---------- medical ----------
      const medicalBlocks = insuredList.map(ins => {
        const m = (p.medical && p.medical[ins.id]) ? p.medical[ins.id] : {};
        const keys = Object.keys(m || {});
        if(!keys.length) return '';
        return `
          <div class="block">
            <div class="h2">שאלון רפואי – ${esc(ins.label)}</div>
            <table class="t">
              <thead><tr><th>סעיף</th><th>ערך</th></tr></thead>
              <tbody>
                ${keys.sort().map(k => `
                  <tr>
                    <td>${esc(k==='DEFAULT|MEDICAL' ? 'שאלון רפואי (ברירת מחדל)' : k.replace('|',' • '))}</td>
                    <td>${esc(m[k])}</td>
                  </tr>
                `).join('')}
              </tbody>
        ...
        `;
      }).filter(Boolean).join('') || `<div class="muted">אין תשובות לשאלון רפואי.</div>`;
      ...
      <head>
        <meta charset="utf-8"/>
        <title>דו״ח תפעולי - ${esc(customerName)}</title>
        <style>
          ...
          .sub{color:#666; margin:0 0 10px; font-size:12px}
          .kpi{display:flex; gap:10px; flex-wrap:wrap; margin:10px 0 14px}
          .kpi .box{border:1px solid #e7e3d6; border-radius:10px; padding:8px 10px; min-width:180px}
          .kpi .lbl{color:#666; font-size:11px}
          .kpi .val{font-size:16px; font-weight:700}
          .kv{width:100%; border-collapse:collapse; margin:8px 0 12px; font-size:12px}
          .kv td{border:1px solid #eee; padding:6px 8px; vertical-align:top}
          .block{margin:14px 0}
          .h2{font-size:14px; font-weight:800; margin:0 0 6px}
          .t{width:100%; border-collapse:collapse; font-size:12px}
          .t th,.t td{border:1px solid #eee; padding:6px 8px; vertical-align:top}
          .t thead th{background:#faf8f1}
      ...
      <body>
        <div class="wrap">
          <h1>דו״ח תפעולי (פנימי)</h1>
          <div class="sub">לקוח: <b>${esc(customerName)}</b> · תאריך: ${esc(createdAt)} · נציג: ${esc(agentName)}</div>

          <table class="kv">
            <tbody>
              <tr>
                <td><b>ת״ז</b><br>${esc(customerTz)}</td>
                <td><b>טלפון</b><br>${esc(customerPhone)}</td>
                <td><b>אימייל</b><br>${esc(customerEmail)}</td>
                <td><b>סטטוס</b><br>${esc(p.status)}</td>
                <td><b>סטטוס פנימי</b><br>${esc(p.internalStatus)}</td>
              </tr>
            </tbody>
          </table>

          <div class="kpi">
            <div class="box"><div class="lbl">סה״כ פרמיה קיימת</div><div class="val">${fmtMoney(totalOld)}</div></div>
            <div class="box"><div class="lbl">סה״כ פרמיה חדשה (לפני)</div><div class="val">${fmtMoney(totalNewBefore)}</div></div>
            <div class="box"><div class="lbl">סה״כ הנחות</div><div class="val">${fmtMoney(totalNewDiscount)}</div></div>
            <div class="box"><div class="lbl">סה״כ פרמיה חדשה (אחרי)</div><div class="val">${fmtMoney(totalNewAfter)}</div></div>
          </div>

          <div class="block">
            <div class="h2">מבוטחים</div>
            <table class="t">
              <thead>
                <tr>
                  <th>מבוטח</th><th>שם</th><th>ת״ז</th><th>נייד</th><th>אימייל</th>
                  <th>ת.לידה</th><th>מין</th><th>קופ״ח</th><th>שב״ן</th>
                  <th>גובה</th><th>משקל</th><th>BMI</th><th>עיסוק</th>
                </tr>
              </thead>
          ...
          <div class="block">
            <div class="h2">פוליסות קיימות</div>
            <table class="t">
              <thead><tr><th>מבוטח</th><th>חברה</th><th>מוצר</th><th class="num">פרמיה</th></tr></thead>
              <tbody>${oldRows}</tbody>
            </table>
          </div>

          <div class="block">
            <div class="h2">פוליסות חדשות</div>
            <table class="t">
              <thead>
                <tr>
                  <th>מבוטח</th><th>חברה</th><th>מוצר</th>
                  <th class="num">לפני</th><th class="num">אחרי</th><th class="num">הנחה</th><th>פירוט הנחה</th>
                </tr>
              </thead>
              <tbody>${newRows}</tbody>
            </table>
          ...
          <div class="block">
            <div class="h2">שאלון רפואי</div>
            ${medicalBlocks}
          </div>

          ${(p.notes || p.summary) ? `<div class="block"><div class="h2">הערות / סיכום</div><div>${esc(p.notes || p.summary)}</div></div>` : ''}

          <script>
            window.onload = () => { try{ window.focus(); }catch(e){} };
          </script>
        </div>
      </body>
      </html>`;
      ...
      console.error(e);
      try{ w.close(); }catch(_){}
      toast('שגיאה', 'נכשלה יצירת תצוגת הדפסה');
    }
  }
