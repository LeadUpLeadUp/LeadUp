  ...
  try{
    ...
    if(isStandalone){
      document.addEventListener('DOMContentLoaded', () => {
        ...
        // Provide inverse scale so fixed overlays (like JOFI) won't get cropped in PWA windows.
        try{
          const v = getComputedStyle(document.body).getPropertyValue('--ui-scale').trim();
          const s = parseFloat(v);
          if(s && isFinite(s) && s > 0){
            document.documentElement.style.setProperty('--ui-scale-inv', (1/s).toFixed(4));
          }
        }catch(_){ }
      });
    }
...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  const JOFI_TARGETS_LS_KEY = 'gemel_jofi_targets_v1';
  const JOFI_DEFAULT_TARGETS = { daily: 500, monthly: 2500 };
  ...
  function fmtNum_(n){
    return Math.round(_num_(n)).toLocaleString('he-IL');
  }
  function getJofiTargetsAll_(){
    try{
      const raw = localStorage.getItem(JOFI_TARGETS_LS_KEY);
  ...
  function setJofiTargetsAll_(obj){
    try{ localStorage.setItem(JOFI_TARGETS_LS_KEY, JSON.stringify(obj||{})); }catch(_){}
  }
  function getJofiTargetsFor_(username){
    const all = getJofiTargetsAll_();
    const t = all && all[username] ? all[username] : null;
    return {
      daily: _num_((t && t.daily)!=null ? t.daily : JOFI_DEFAULT_TARGETS.daily),
      monthly: _num_((t && t.monthly)!=null ? t.monthly : JOFI_DEFAULT_TARGETS.monthly)
    };
  }
  function setJofiTargetsFor_(username, targets){
    const all = getJofiTargetsAll_();
    all[username] = { daily:_num_(targets.daily), monthly:_num_(targets.monthly) };
    setJofiTargetsAll_(all);
  }
  function resetJofiTargetsFor_(username){
    const all = getJofiTargetsAll_();
    if(all && all[username]){ delete all[username]; setJofiTargetsAll_(all); }
  }
  ...
  function calcJofiSales_(username, displayName){
    const todayKey = dateKey_(new Date());
    const thisMonthKey = monthKey_(new Date());

    let daily = 0;
    let monthly = 0;

    const identifiers = new Set([username, displayName].filter(Boolean));

    (state.proposals || []).forEach(p => {
      const pAssigned = p && p.assignedAgent ? String(p.assignedAgent) : '';
      const pCreatedAt = p && p.createdAt ? p.createdAt : null;

      const newPolicies = Array.isArray(p && p.newPolicies) ? p.newPolicies : [];
      newPolicies.forEach(rec => {
        const prem = _num_(rec && rec.premium);
        if(prem <= 0) return;

        const recCreatedAt = (rec && (rec.createdAt || rec.created_at)) ? (rec.createdAt || rec.created_at) : pCreatedAt;
        const dk = dateKey_(recCreatedAt || new Date());
        const mk = monthKey_(recCreatedAt || new Date());

        const recBy = rec && (rec.createdBy || rec.created_by) ? (rec.createdBy || rec.created_by) : null;

        // Ownership: prefer explicit createdBy; fallback to proposal assignedAgent match
        let mine = false;
        if(recBy){
          mine = identifiers.has(String(recBy));
        }else{
          mine = identifiers.has(pAssigned);
        }

        if(!mine) return;

        if(dk === todayKey) daily += prem;
        if(mk === thisMonthKey) monthly += prem;
      });
    });
  ...
  function setGauge_(scope, value, target){
    const root = jofiOverlayEl ? jofiOverlayEl.querySelector('.jofiGauge[data-scope="'+scope+'"]') : null;
    if(!root) return;
    const prog = root.querySelector('.jofiProg');
    if(!prog) return;
    ...
    prog.style.strokeDasharray = dash + ' ' + (C + 5);

    if(scope === 'daily'){
      $('#jofiDailyVal').textContent = fmtNum_(value);
      $('#jofiDailySub').textContent = 'יעד: ' + fmtNum_(target);
      $('#jofiDailyPct').textContent = Math.round(pct*100) + '%';
    }else{
      $('#jofiMonthlyVal').textContent = fmtNum_(value);
      $('#jofiMonthlySub').textContent = 'יעד: ' + fmtNum_(target);
      $('#jofiMonthlyPct').textContent = Math.round(pct*100) + '%';
    }
  }
  ...
  function openJofi_(){
    if(!jofiOverlayEl) return;
    jofiOverlayEl.classList.add('isOpen');
    jofiOverlayEl.setAttribute('aria-hidden','false');

    // admin panel
    const adminWrap = $('#jofiAdmin');
    if(adminWrap){
      adminWrap.style.display = isAdmin_() ? '' : 'none';
    }
    if(isAdmin_()){
      try{ buildJofiAdmin_(); }catch(_){}
    }

    refreshJofi_();
    if(_jofiTimer) clearInterval(_jofiTimer);
    _jofiTimer = setInterval(refreshJofi_, 15000);
  }

  function closeJofi_(){
    if(!jofiOverlayEl) return;
    jofiOverlayEl.classList.remove('isOpen');
    jofiOverlayEl.setAttribute('aria-hidden','true');
    if(_jofiTimer){ clearInterval(_jofiTimer); _jofiTimer = null; }
  }

  function refreshJofi_(){
    const u = state.currentUser || {};
    const username = u.username || '';
    const displayName = u.displayName || state.agentName || '';

    const t = getJofiTargetsFor_(username);
    const sales = calcJofiSales_(username, displayName);

    setGauge_('daily', sales.daily, t.daily);
    setGauge_('monthly', sales.monthly, t.monthly);
  }

  function buildJofiAdmin_(){
    const sel = $('#jofiUserSelect');
    const dailyInp = $('#jofiDailyTarget');
    const monthlyInp = $('#jofiMonthlyTarget');
    const saveBtn = $('#jofiSaveTargets');
    const resetBtn = $('#jofiResetTargets');
    if(!sel || !dailyInp || !monthlyInp || !saveBtn || !resetBtn) return;

    const users = (getUsers_() || []).filter(u => u && u.active !== false);
    sel.innerHTML = users.map(u => {
      const name = (u.displayName || u.username);
      const roleTag = (u.role === 'admin') ? ' (מנהל)' : '';
      return '<option value="'+escapeHtml_(u.username)+'">'+escapeHtml_(name + roleTag)+'</option>';
    }).join('');

    const applyForUser = () => {
      const uname = sel.value;
      const tt = getJofiTargetsFor_(uname);
      dailyInp.value = Math.round(_num_(tt.daily));
      monthlyInp.value = Math.round(_num_(tt.monthly));
    };

    sel.onchange = applyForUser;
    applyForUser();

    saveBtn.onclick = () => {
      const uname = sel.value;
      setJofiTargetsFor_(uname, { daily: _num_(dailyInp.value), monthly: _num_(monthlyInp.value) });
      toast('נשמר', 'היעדים עודכנו');
      refreshJofi_();
    };

    resetBtn.onclick = () => {
      const uname = sel.value;
      resetJofiTargetsFor_(uname);
      toast('אופס', 'אופס לברירת מחדל');
      applyForUser();
      refreshJofi_();
    };
  }
  ...
  const jofiBtn = $('#jofiBtn');
  const jofiOverlayEl = $('#jofiOverlay');
  const jofiCloseBtn = $('#jofiCloseBtn');
  ...
  window.gemelLogout = function(){
    // Ensure overlays are closed so nothing "leaks" over the login screen
    try{ closeJofi_(); }catch(_){ }
    clearSession_();
...
function openUserModal_(opt){
  ...
  if(jofiBtn){ jofiBtn.addEventListener('click', openJofi_); }
  if(jofiCloseBtn){ jofiCloseBtn.addEventListener('click', closeJofi_); }
  if(jofiOverlayEl){
    jofiOverlayEl.addEventListener('click', (e) => { if(e.target === jofiOverlayEl) closeJofi_(); });
    document.addEventListener('keydown', (e) => {
      if(e.key === 'Escape' && jofiOverlayEl.classList.contains('isOpen')) closeJofi_();
    });
  }
...
function renderNewPoliciesStep(){
    ...
    $('#addNewBtn').addEventListener('click', () => {
      ...
      const rec = { insuredId, company, product, premium, createdAt: new Date().toISOString(), createdBy: (state.currentUser && state.currentUser.username) ? state.currentUser.username : null, createdByName: state.agentName || null };
