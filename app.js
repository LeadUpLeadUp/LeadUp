...
/* =======================
   ...
   - By default: seed ONCE if there are no leads yet (clean environment)
   - If demoAlways=true: MERGE demo into existing data (no overwrites, no duplicates)
   ======================= */
const DEMO_SEEDED_KEY = "leadup_demo_seeded_v1";

function seedDemoData(mode="once"){
  ...
  if(!merge){
    try{ if(localStorage.getItem(DEMO_SEEDED_KEY)==="1") return; }catch(e){}
  ...
  const mkLead = (id, o)=>({
    ...
    policies: Array.isArray(o.policies) ? o.policies : [],
  ...
  const demoLeads = [
    mkLead("lead_demo_1", {
      firstName:"יואב", lastName:"אברהם", phone:"050-1234567", email:"yoav@example.com", tz:"123456789",
      source:"גוגל", status:"חדש", assignedTo:"דנה לוי",
      note:"דמו: ליד חדש נכנס, טרם שיחה.",
      nextFollowUpAt: new Date(now+3600*1000*6).toISOString(),
      policies:[
        {id:"p1", type:"ביטוח בריאות", company:"הראל", premium:220, startAt:"2025-10-01"},
        {id:"p2", type:"ביטוח חיים", company:"מגדל", premium:180, startAt:"2024-06-01"}
      ],
      docs:[
        {id:"d1", name:"כיסויים הראל - יוני 2024", url:"https://example.com/harel.pdf", note:"דמו"},
        {id:"d2", name:"טופס הצעה - מגדל", url:"https://example.com/migdal.pdf", note:"דמו"}
      ]
    }),
    mkLead("lead_demo_2", {
      firstName:"נועה", lastName:"כהן", phone:"052-9876543", email:"noa@example.com", tz:"234567891",
      source:"פייסבוק", status:"מתעניין", assignedTo:"יוסי כהן",
      note:"דמו: שיחה ראשונית, ביקשה הצעת מחיר.",
      nextFollowUpAt: new Date(now+3600*1000*24).toISOString()
    }),
    mkLead("lead_demo_3", {
      firstName:"אור", lastName:"לוי", phone:"054-7778899", email:"or@example.com", tz:"345678912",
      source:"הפניה", status:"הצעת מחיר", assignedTo:"נועה ישראלי",
      note:"דמו: נשלחה הצעה, ממתין לתשובה."
    })
  ];

  appState.leads = Array.isArray(appState.leads) ? appState.leads : [];
  const byId = new Set(appState.leads.map(l=>l?.id).filter(Boolean));
  const toAdd = demoLeads.filter(l=>!byId.has(l.id));
  if(toAdd.length){
    appState.leads.unshift(...toAdd);
    try{ addEvent("demo_seed", `נטענו נתוני דמו (${toAdd.length})`, {}); }catch(e){}
  }

  try{ localStorage.setItem(DEMO_SEEDED_KEY,"1"); }catch(e){}
}

function isDemoAlways(){
  try{
    const c = loadCfg();
    return c && c.demoAlways !== false;
  }catch(e){
    return true;
  }
}
...
function loadCfg(){
  try{
    ...
    return {
      ...
      demoAlways: cfg.demoAlways !== false
    };
  }catch(e){
    return { mode:"local", webAppUrl:"", autoSync:true, lastOkAt:null, demoAlways:true };
  }
}
...
function normQ_(s){ return String(s||"").trim().toLowerCase(); }
function digitsOnly_(s){ return String(s||"").replace(/\D/g,""); }
function findLeadAuto(q){
  const qq = normQ_(q);
  const qd = digitsOnly_(q);
  if(!qq) return null;
  const leads = (appState.leads || []);
  if(qd.length>=8){
    const byTz = leads.find(l=>{ const v = digitsOnly_(l.tz || l.idNumber || ""); return v && v===qd; });
    if(byTz) return byTz;
  }
  if(qd.length>=6){
    const byPhone = leads.find(l=>{ const p1 = digitsOnly_(l.phone||""); const p2 = digitsOnly_(l.phone2||""); return (p1 && p1.includes(qd)) || (p2 && p2.includes(qd)); });
    if(byPhone) return byPhone;
  }
  const byName = leads.find(l=> normQ_(l.name).includes(qq) || normQ_(l.firstName+" "+l.lastName).includes(qq));
  return byName || null;
}

(function wireGlobalSearch(){
  const inp = el("globalSearchQ");
  const btn = el("globalSearchBtn");
  if(!inp || !btn) return;
  const run = ()=>{
    const q = (inp.value||"").trim();
    if(!q){ toast("הקלד שם / טלפון / ת.ז", "warn"); inp.focus(); return; }
    const lead = findLeadAuto(q);
    if(!lead){
      if((appState.leads||[]).length===0){
        toast("אין לידים במערכת. עבור להגדרות וטען דמו.", "warn");
      }else{
        toast("לא נמצא לקוח", "warn");
      }
      return;
    }
    openClientDossier(lead.id);
  };
...
function systemCheck(){
  try{
    const cfg = loadCfg();
    const leadsCount = Array.isArray(appState.leads) ? appState.leads.length : 0;
    const tasksCount = Array.isArray(appState.tasks) ? appState.tasks.length : 0;
    const eventsCount = Array.isArray(appState.events) ? appState.events.length : 0;
    ...
    const out = `מצב: ${cfg.mode} • לידים: ${leadsCount} • נציגים: ${usersCount} • משימות: ${tasksCount} • אירועים: ${eventsCount}`;
    const elOut = el("systemCheckOut");
    if(elOut) elOut.textContent = out;

    console.log("[LEADUP SystemCheck]", {
      mode: cfg.mode,
      leadsCount, usersCount, tasksCount, eventsCount,
      sampleLead: (appState.leads && appState.leads[0]) ? appState.leads[0] : null,
      build: (window.__LEADUP_BUILD || null)
    });

    if(leadsCount===0){
      toast("אין לידים. נסה להוסיף דמו או ליצור ליד חדש.", "warn");
    }else{
      toast("בדיקת מערכת: תקין ✅", "success");
    }
  }catch(e){
    console.error("systemCheck failed", e);
    toast("בדיקת מערכת נכשלה", "error");
  }
}

(function wireSystemCheck(){
  const b = el("btnSystemCheck");
  if(!b) return;
  b.onclick = systemCheck;
})();
})();
...
function importJSON(){
  ...
  inp.onchange = () => {
    ...
    toast("ליד נשמר בהצלחה ✅", "success");
      // afterLeadSaveGoLeads_v1
      try{ el("q").value=""; el("fStatus").value=""; el("fSource").value=""; el("sortBy").value="newest"; }catch(e){}
      try{ showView("leads"); }catch(e){}
...
/* =======================
   ...
   ======================= */
(async function init(){
  try{ if(isDemoAlways()) seedDemoData("merge"); else seedDemoData("once"); }catch(e){}
  ...
  wireGsUI();
  // Demo toggle/buttons (Settings)
  try{
    const demoChk = el("demoAlwaysChk");
    if(demoChk){
      demoChk.checked = (loadCfg().demoAlways !== false);
      demoChk.onchange = ()=>{
        appCfg = saveCfg({ demoAlways: demoChk.checked });
        toast(demoChk.checked ? "דמו תמיד פעיל ✅" : "דמו תמיד כבוי", "success");
        if(demoChk.checked){
          seedDemoData("merge");
          saveState();
          renderAll();
    ...
    const demoAdd = el("demoAddBtn");
    if(demoAdd){
      demoAdd.onclick = ()=>{
        seedDemoData("merge");
        saveState();
        renderAll();
        toast("דמו נוסף (מיזוג) ✅", "success");
      };
    }
  }catch(e){}
