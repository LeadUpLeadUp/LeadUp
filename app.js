...
// Default Web App URL (can be overridden in Settings)
const DEFAULT_WEBAPP_URL = "https://script.google.com/macros/s/AKfycbxfu_Qe4sse-g_6GS1aVfPMAM0Xv4XSsx28ANi0Z8PZy8SW6zzxOTzCQvLsomKZLoVppw/exec";
function loadCfg(){
  try{
    ...
    return {
      ...
      webAppUrl: ((cfg.webAppUrl || "").trim() || DEFAULT_WEBAPP_URL),
...
async function gsPush(){
  ...
  // Preferred protocol (NEW): POST /exec?action=save with JSON body
  const payload = {
    // store the whole app state so we can restore everything (leads/tasks/events/sequences)
    meta: { app:"LEADUP", build: (typeof BUILD_ID!=="undefined"?BUILD_ID:""), savedAt: nowISO() },
    leads: appState.leads || [],
    tasks: appState.tasks || [],
    events: appState.events || [],
    leadsSeq: appState.leadsSeq || 1,
    tasksSeq: appState.tasksSeq || 1
  };

  try{
    const data = await gsFetch(url + "?action=save", {
      method:"POST",
      headers:{ "Content-Type":"application/json;charset=utf-8" },
      body: JSON.stringify(payload)
    });
    if(!data || data.ok !== true) throw new Error(data?.error || "SAVE_FAILED");
    appCfg = saveCfg({ lastOkAt: nowISO() });
    return data;
  }catch(e){
    // Fallback (LEGACY): some older scripts expect POST to /exec with action+payload in body
    const legacyBody = JSON.stringify({
      action: "put",
      payload: payload,
      meta: { app:"LEADUP", version:(typeof BUILD_ID!=="undefined"?BUILD_ID:"legacy") }
    });

    const data2 = await gsFetch(url, {
      method:"POST",
      headers:{ "Content-Type":"text/plain;charset=utf-8" },
      body: legacyBody
    });
    if(!data2 || data2.ok !== true) throw new Error("שמירה לשיט נכשלה");
    appCfg = saveCfg({ lastOkAt: nowISO() });
    return data2;
  }
}
