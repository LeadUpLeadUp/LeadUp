...
const DEFAULT_SHEETS_WEBAPP_URL = "https://script.google.com/macros/s/AKfycby7PVYei5sW9wRoEy7LYb4L_gw9EuOM1JzY_TF6zXU3FfXLztCfgMnaEj3s81n9iliLzA/exec";
...
function loadCfg(){
  try{
    ...
    return {
      ...
      webAppUrl: (cfg.webAppUrl || DEFAULT_SHEETS_WEBAPP_URL || "").trim(),
      autoSync: cfg.autoSync !== false,
      lastOkAt: cfg.lastOkAt || null
    };
  }catch(e){
    return { mode:"local", webAppUrl:(DEFAULT_SHEETS_WEBAPP_URL||""), autoSync:true, lastOkAt:null };
  }
}
...
async function gsPull(){
  ...
  // 1) {ok:true, payload:{leads:[], tasks:[], events:[]}}
  // 2) {ok:true, payload:{payload:{...}}} (legacy wrappers)
  const p1 = (data.payload && typeof data.payload === "object") ? data.payload : {};
  const next = (p1.payload && typeof p1.payload === "object") ? p1.payload : p1;
...
async function gsPush(){
  const base = normalizeWebAppUrl(appCfg.webAppUrl);
  if(!base) throw new Error("חסר URL של Web App");

  // Preferred protocol (matches Apps Script package): POST ?action=save with JSON body = full state
  try{
    const data = await gsFetch(base + "?action=save", {
      method:"POST",
  ...
  // Fallback protocol (older deployments): POST /exec with wrapper payload
  const legacyBody = JSON.stringify({
    action: "put",
    payload: appState,
    meta: { app:"LEADUP", version: BUILD_ID || "unknown" }
  });
  const legacy = await gsFetch(base, {
    method:"POST",
    headers: { "Content-Type":"text/plain;charset=utf-8" },
    body: legacyBody
  });
  if(!legacy || legacy.ok !== true) throw new Error("שמירה לשיט נכשלה");
  appCfg = saveCfg({ lastOkAt: nowISO() });
  return legacy;
}
