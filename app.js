(() => {
  ...
  function exportCasePDF(c){
    ...
    const qHtml = (c.products||[]).map((p, i)=>{
      ...
      return `
        <div class="blk">
          <div class="blkTitle">שאלון • ${esc(p.company||"—")} • ${esc(p.productName||"—")} <span class="mono">(${esc(p.id)})</span></div>
          <table class="t">
...
<body>
  <div class="page">
    ...
    <div class="blk">
      ...
      <table class="t">
        <thead><tr><th>#</th><th>חברה</th><th>מוצר</th><th>פרמיה לפני</th><th>פרמיה אחרי</th></tr></thead>
        <tbody>
          ${(c.products||[]).length ? productsHtml : `<tr><td colspan="5">אין מוצרים</td></tr>`}
        </tbody>
      </table>
    </div>
  ...
  function renderSettings(){
    ...
    app.innerHTML = `
      <div class="card">
        <div class="cardTitle">בקרוב</div>
        <div class="cardMeta">מסך הגדרות מלא. כרגע אתה עובד במצב מקומי בלבד.</div>
      </div>
    `;
  }
  // ----- Persistence -----
