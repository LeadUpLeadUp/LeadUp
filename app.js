...
(function(){
  ...
  async function registerAndUpdate(){
    ...
    try{
      const reg = await navigator.serviceWorker.register(`./service-worker.js?build=${b}`);
      // Force an update check on every load so UI/login changes show up immediately
      ...
      reg.addEventListener("updatefound", () => {
        ...
        nw.addEventListener("statechange", () => {
          // When installed and there's an existing controller, new version is ready
      ...
      // When the controller changes, reload so the new cache/logic takes effect
