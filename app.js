...
(() => {
  ...
  // ---------- Login UI ----------
  function showLogin_(){
    // Safety: if for any reason the overlay element is missing in the DOM,
    // do NOT lock the whole app (pointer-events:none). Instead, keep the app usable.
    if(!loginOverlay){
      console.warn('[Auth] loginOverlay missing in DOM - skipping lock screen');
