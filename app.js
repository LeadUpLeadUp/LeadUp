...
/* =======================
   Demo tools (Settings buttons)
   ======================= */
function seedDemoUsersOnly(){
  try{
    const raw = localStorage.getItem(USERS_KEY);
    const arr = raw ? JSON.parse(raw) : [];
    const users = Array.isArray(arr) ? arr : [];
    ...
    // Admin (ensure)
    ensureById({ id:"u_admin", name:"מנהל מערכת", pin:"3316", role:"Admin", active:true, createdAt: Date.now()-86400000 });

    // Demo agents
    ensureById({ id:"u_demo_1", name:"דנה לוי", pin:"1111", role:"Agent", active:true, createdAt: Date.now()-7200000 });
    ensureById({ id:"u_demo_2", name:"יוסי כהן", pin:"2222", role:"Agent", active:true, createdAt: Date.now()-5400000 });
    ensureById({ id:"u_demo_3", name:"נועה ישראלי", pin:"3333", role:"Agent", active:true, createdAt: Date.now()-3600000 });

    localStorage.setItem(USERS_KEY, JSON.stringify(users));
    toast("נציגים דמו נטענו ✅", "success");
    try{ renderAll(); }catch(e){}
  }catch(e){
    toast("שגיאה בטעינת נציגים דמו", "error");
  }
}

function clearDemoData(){
  try{
    // Remove demo users by id
    try{
      const raw = localStorage.getItem(USERS_KEY);
      const arr = raw ? JSON.parse(raw) : [];
      const users = Array.isArray(arr) ? arr : [];
      const filtered = users.filter(u => u && !String(u.id||"").startsWith("u_demo_"));
      localStorage.setItem(USERS_KEY, JSON.stringify(filtered));
    }catch(e){}

    // Remove demo leads by email domain and note marker (best-effort, safe)
    if(Array.isArray(appState.leads) && appState.leads.length){
      appState.leads = appState.leads.filter(l=>{
        const email = String(l.email||"").toLowerCase();
        const note = String(l.note||"").toLowerCase();
        // our seeded emails are *@example.com and notes contain "דמו:"
        if(email.endsWith("@example.com")) return false;
        if(note.includes("דמו:")) return false;
        return true;
      });
      addEvent("seed", "נוקו נתוני דמו");
      saveState();
    }else{
      try{ saveState(); }catch(e){}
    }

    try{ localStorage.removeItem(DEMO_SEEDED_KEY); }catch(e){}
    toast("דמו נוקה ✅", "success");
  }catch(e){
    toast("שגיאה בניקוי דמו", "error");
  }
}

function wireDemoButtons(){
  el("btnDemoSeedUsers")?.addEventListener("click", ()=>{
    if(confirm("לטעון נציגים דמו? (לא מוחק נתונים קיימים)")){
      seedDemoUsersOnly();
    }
  });

  el("btnDemoSeed")?.addEventListener("click", ()=>{
    if(confirm("לטעון לידים + נציגים דמו? (ייטען רק אם אין עדיין לידים)")){
      // Ensure demo agents first
      seedDemoUsersOnly();
      // Seed leads only if empty (safe)
      try{ seedDemoDataOnce(); }catch(e){}
      try{ renderAll(); }catch(e){}
    }
  });

  el("btnDemoClear")?.addEventListener("click", ()=>{
    if(confirm("לנקות את נתוני הדמו? (ימחק לידים/נציגים דמו בלבד)")){
      clearDemoData();
      try{ renderAll(); }catch(e){}
    }
  });
...
/* =======================
   ...
   ======================= */
(async function init(){
  /* AUTO_DEMO_SEED_ON_EMPTY */
  try{
    if(appCfg && appCfg.mode==="local" && (!Array.isArray(appState.leads) || appState.leads.length===0)){
      seedDemoDataOnce();
    }
  }catch(e){}
  ...
  wireGsUI();
  // Demo buttons in Settings
  try{ wireDemoButtons(); }catch(e){}
