...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  function openEditConfirmModal_({ onConfirm }){
    const overlay = document.createElement('div');
    overlay.className = 'editConfirmOverlay';
    overlay.innerHTML = `
      <div class="editConfirmModal" role="dialog" aria-modal="true" aria-label="אישור שמירת עריכה">
        <div class="editConfirmTitle">שמור עריכת הצעה</div>
        <div class="editConfirmHint">אנא לרשום מה בוצע/שונה בהצעה</div>

        <div class="editConfirmRow">
          <label for="editNote">מה שונה בהצעה? (חובה)</label>
          <textarea id="editNote" placeholder="לדוגמה: עודכן סכום ביטוח בריסק, נוספה פוליסה חדשה, עודכן טלפון לקוח..."></textarea>
        </div>

        <div class="editConfirmCheck">
          <input id="editApprove" type="checkbox">
          <label for="editApprove">האם אתה מאשר את התיקונים שערכת בהצעה?</label>
        </div>

        <div class="editConfirmActions">
          <button class="btn btnSoft" data-cancel>ביטול</button>
          <button class="btn" data-ok>מאשר ושומר</button>
        </div>
      </div>
    `;
    ...
    overlay.querySelector('[data-ok]')?.addEventListener('click', () => {
      const note = String(overlay.querySelector('#editNote')?.value || '').trim();
      const ok = !!overlay.querySelector('#editApprove')?.checked;
      if(!note){
        toast('חסר', 'חובה לרשום מה שונה בהצעה');
        overlay.querySelector('#editNote')?.focus();
        return;
      }
      if(!ok){
        toast('אישור', 'יש לסמן וי לאישור התיקונים');
        return;
      }
      close();
      try{ onConfirm && onConfirm({ note }); }catch(_){}
    });

    setTimeout(() => overlay.querySelector('#editNote')?.focus(), 50);
  }
...
function openUserModal_(opt){
  ...
  function renderCustomersTable(rows){
          ...
          <tbody>
            ${rows.map(c => `
              <tr>
                ...
                <td style="text-align:left;white-space:nowrap">
                      <button class="btn btnSoft" data-open-proposal="${c.id}">${latestProposalForCustomer(c.id)? "ערוך הצעה" : "צור הצעה"}</button>
    ...
    $$('[data-open-proposal]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if(lp){
          openWizard({ proposalId: lp.id, customerId: cid, mode: 'edit' });
        }else{
  ...
  if(saveEditBtn){
    saveEditBtn.addEventListener('click', () => {
      if(!wiz || !wiz.isEdit) return;
      openEditConfirmModal_({
        onConfirm: ({ note }) => {
          const orig = wiz.originalProposal;
          const draft = wiz.proposal;
          if(!orig || !draft) return;
          const idx = state.proposals.findIndex(p => p.id === orig.id);
          if(idx < 0) return;

          const history = Array.isArray(orig.editHistory) ? orig.editHistory : [];
          const entry = { at: new Date().toISOString(), by: state.agentName || '', note: String(note||'').trim() };

          const committed = { ...draft, id: orig.id, createdAt: orig.createdAt || draft.createdAt };
          committed.updatedAt = new Date().toISOString();
          committed.editHistory = [entry, ...history];

          state.proposals[idx] = committed;
          toast('נשמר', 'עריכת הצעה נשמרה');
          closeWizard();
          render();
        }
      });
  ...
  function openWizard({ customerId=null, proposalId=null, mode=null }){
    ...
    if(proposalId){
      originalProposal = state.proposals.find(p => p.id === proposalId) || null;
      if(originalProposal){
        // Edit mode: work on a draft copy until user confirms save
        proposal = deepClone_(originalProposal);
      }
    }
