(() => {
  ...
  const HOME_CARDS = document.querySelectorAll('.home-actions .entry-card');
  ...
  function finishIntro() {
    ...
    HOME_ACTIONS?.classList.add('is-showcase');
    HOME_CARDS?.forEach(card => card.classList.add('is-showcase'));
    window.setTimeout(() => {
      HOME_ACTIONS?.classList.remove('is-showcase');
      HOME_CARDS?.forEach(card => card.classList.remove('is-showcase'));
    }, 5200);
    setScreen('home');
  }
