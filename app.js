...
function seedDemoDataOnce(){
  // Idempotent demo seeding: only once, and only when there are no real leads yet.
  try{
  ...
  // --- Ensure demo agents in login users ---
  try{
    const raw = localStorage.getItem(USERS_KEY);
    const arr = raw ? JSON.parse(raw) : [];
    ...
    const ensureUser = (u)=>{
      if(!u || !u.id) return;
      if(!users.some(x=>x && x.id===u.id)) users.push(u);
    };
...
/* ===== Activity Timeline ===== */
function addEvent(type, text, leadIdOrMeta=null, metaMaybe=null){
  // Backward compatible:
  // - addEvent(type, text, metaObj)
  // - addEvent(type, text, leadId)
  // - addEvent(type, text, leadId, metaObj)
  let leadId = null;
  let meta = {};
  if(leadIdOrMeta && typeof leadIdOrMeta === "object" && !Array.isArray(leadIdOrMeta)){
    meta = leadIdOrMeta;
  }else if(typeof leadIdOrMeta === "string"){
    leadId = leadIdOrMeta;
    if(metaMaybe && typeof metaMaybe === "object" && !Array.isArray(metaMaybe)) meta = metaMaybe;
  }else if(leadIdOrMeta != null){
    meta = { value: leadIdOrMeta };
  }
  ...
  const ev = {
    ...
    type,
    text,
    msg: text,          // legacy renderer expects msg
    leadId: leadId || meta.leadId || null,
    meta
  };
...
function setCustomerTab(tab){
  ...
  const tabs = ["details","flows","docs","sign","history"];
  tabs.forEach(t=>{
    const b = el("ctab_"+t);
...
function renderCustomerHistory(){
  ...
  box.innerHTML = events.map(ev=>`
    <div class="event">
      ...
      <div class="d">${escapeHTML((ev.msg||ev.text||""))}</div>
    </div>
  `).join("");
}
...
function wireCustomerView(){
  ...
  el("btnCustomerSave")?.addEventListener("click", ()=>{
    ...
    addEvent("edit", "עודכנו פרטי לקוח", lead.id);
    saveState();
    toast("נשמר ✅", "success");
    openCustomerView(lead); // refresh header
  });

  el("btnPolicyAdd")?.addEventListener("click", ()=>{
    const lead = getCustomerLead();
    if(!lead) return;
    const name = safeVal("cd_pol_name","").trim();
    const prem = Number(safeVal("cd_pol_premium","0"));
    if(!name){ toast("הכנס שם פוליסה", "warn"); return; }
    _cdEnsureLeadCollections(lead);
    lead.policies.push({ name, premium: isFinite(prem)?prem:0, at: Date.now() });
    safeSet("cd_pol_name",""); safeSet("cd_pol_premium","");
    addEvent("policy", `נוספה פוליסה: ${name}`, lead.id);
    saveState();
    renderCustomerPolicies();
    // update totals
    const total = _cdComputePremiumTotal(lead);
    el("cd_premiumTotal2").textContent = _cdFmtMoney(total);
    el("cd_policiesCount2").textContent = String((lead.policies||[]).length);
  });
  ...
  el("btnDocAdd")?.addEventListener("click", ()=>{
    ...
    addEvent("doc", `נוסף מסמך: ${name}`, lead.id);
    saveState();
    renderCustomerDocs();
  });
  ...
  const signNow = ()=>{
    const lead = getCustomerLead();
    if(!lead) return;
    addEvent("sign", "נשלח מסמך לחתימה (דמו)", lead.id);
    saveState();
    const s = el("cd_sign_status");
    if(s) s.textContent = "נשלח לחתימה ✅ (דמו)";
    toast("נשלח לחתימה ✅", "success");
    renderCustomerHistory();
  };
  el("btnSignSendNow")?.addEventListener("click", signNow);
  el("btnCustomerSendSign")?.addEventListener("click", signNow);
...
function findLeadByQuery(q){
  const s = String(q||"").trim();
  if(!s) return null;
  const digits = s.replace(/\D+/g,"");
  const lower = s.toLowerCase();

  // priority: tz exact, phone contains, name contains
  let lead = null;
  if(digits.length>=7){
    lead = appState.leads.find(l=>String(l.tz||"").replace(/\D+/g,"")===digits) || null;
    if(!lead){
      lead = appState.leads.find(l=>String(l.phone||"").replace(/\D+/g,"").includes(digits)) || null;
    }
    if(!lead){
      lead = appState.leads.find(l=>String(l.phone2||"").replace(/\D+/g,"").includes(digits)) || null;
    }
  }
  if(!lead){
    lead = appState.leads.find(l=>String(l.name||"").toLowerCase().includes(lower)) || null;
  }
  return lead;
}
...
function wireTopSearch(){
  ...
  qEl.addEventListener("keydown", (ev)=>{
    if(ev.key==="Enter"){
      ev.preventDefault();
      const lead = findLeadByQuery(qEl.value);
      if(!lead){
        toast("לא נמצא לקוח", "warn");
        return;
      }
      openCustomerView(lead);
    }
  });
}
...
/* ===== Toast ===== */
let toastTimer=null;
function showLoader(){/*stub*/}
function hideLoader(){/*stub*/}
function openProposalModal(){ toast("הצעות: בפיתוח", "warn"); }
...
/* ===== Filters/Search/Sort ===== */
/* ===== Lead ownership filter (My / All) ===== */
let __leadOwnerFilter = "all"; // "all" | "mine"
function getMyName(){
  const su = getSessionUser && getSessionUser();
  const n = (su && su.name) ? String(su.name).trim() : String(appState.user?.name||"").trim();
  return n;
}
function setOwnerFilter(mode){
  __leadOwnerFilter = (mode === "mine") ? "mine" : "all";
  // Visual state
  const bMine = el("btnMineLeads"), bAll = el("btnAllLeads");
  if(bMine) bMine.classList.toggle("goldSoft", __leadOwnerFilter==="mine");
  if(bAll)  bAll.classList.toggle("goldSoft", __leadOwnerFilter==="all");
  renderLeads();
}
...
// New Lead (topbar + inline)
const _btnNewLead = el("btnNewLead"); 
if(_btnNewLead) _btnNewLead.addEventListener("click", ()=> openLeadForm(null));
const _btnNewLeadInline = el("btnNewLeadInline");
if(_btnNewLeadInline) _btnNewLeadInline.addEventListener("click", ()=> openLeadForm(null));
...
function openLeadForm(leadId=null){
  ...
  <div>
    <label>שם פרטי</label>
    <input name="firstName" value="${escapeHTML(data.firstName||"")}" placeholder="שם פרטי" required/>
  </div>
  <div>
    <label>שם משפחה</label>
    <input name="lastName" value="${escapeHTML(data.lastName||"")}" placeholder="שם משפחה" required/>
  </div>
  <div>
    <label>טלפון</label>
    <input name="phone" value="${escapeHTML(data.phone)}" placeholder="05X-XXXXXXX" inputmode="tel" required/>
  </div>
  <div>
    <label>טלפון נוסף</label>
    <input name="phone2" value="${escapeHTML(data.phone2||"")}" placeholder="" inputmode="tel"/>
  </div>
  <div>
    <label>ת.ז</label>
    <input name="idNumber" value="${escapeHTML(data.idNumber||data.tz||"")}" placeholder="ת.ז" inputmode="numeric"/>
  </div>
</div>
          </div>
  ...
  <select name="assignedTo" id="assignedToSel">
    ${
      (function(){
        const agents = getActiveAgents();
        const cur = (data.assignedTo || data.owner || appState.user?.name || "").trim();
        const fallback = cur || (agents[0] || "");
        const opts = agents.length ? agents : (fallback ? [fallback] : []);
        return (opts.length ? opts : ["מנהל מערכת"])
          .map(n => `<option ${((n===cur)||(n===fallback))?"selected":""}>${escapeHTML(n)}</option>`)
          .join("");
      })()
    }
  </select>
  <div class="help">אפשר לשייך את הליד לנציג אחר להמשך טיפול (שיחה / הצעה / מעקב).</div>
</div>
          </div>
...
<div class="row" style="margin-top:10px">
  <div>
    <label>כתובת</label>
    <input name="address" value="${escapeHTML(data.address||"")}" placeholder="רחוב ומספר"/>
  </div>
  <div>
    <label>עיר</label>
    <input name="city" value="${escapeHTML(data.city||"")}" placeholder="עיר"/>
  </div>
  <div>
    <label>חברה / מקום עבודה</label>
    <input name="company" value="${escapeHTML(data.company||"")}" placeholder="אופציונלי"/>
  </div>
</div>

<div class="row" style="margin-top:10px">
  <div>
    <label>מקור קמפיין</label>
    <input name="campaign" value="${escapeHTML(data.campaign||"")}" placeholder="לדוגמה: פייסבוק - טופס 02/2026"/>
    <div class="help">עוזר להבין מאיפה הגיע הליד (מודעה/קמפיין/שותף).</div>
  </div>
</div>
...
/* LEADUP_PATCH: lead_form_normalize_validate_v1 */
// Normalize name
updated.firstName = String(updated.firstName||"").trim();
updated.lastName  = String(updated.lastName||"").trim();
updated.name = (updated.firstName + " " + updated.lastName).trim() || String(updated.name||"").trim();

// Normalize ID
const _id = String(updated.idNumber||updated.tz||"").trim();
if(_id){ updated.idNumber = _id; updated.tz = _id; } else { updated.idNumber=""; updated.tz=""; }

// Assignment (keep legacy owner)
updated.assignedTo = String(updated.assignedTo||updated.owner||"").trim();
if(!updated.assignedTo){
  try{
    const su = getSessionUser();
    if(su && su.name) updated.assignedTo = String(su.name).trim();
  }catch(e){}
}
if(!updated.assignedTo && appState.user?.name) updated.assignedTo = String(appState.user.name).trim();
updated.owner = updated.assignedTo;

// Basic cleanup
updated.phone = String(updated.phone||"").trim();
updated.phone2 = String(updated.phone2||"").trim();
updated.email = String(updated.email||"").trim();

// Smart validation
const errs = [];
const phoneDigits = String(updated.phone||"").replace(/\D/g,"");
if(!(phoneDigits.length>=9 && phoneDigits.length<=10)) errs.push("טלפון חובה (9–10 ספרות)");
const idDigits = String(updated.idNumber||"").replace(/\D/g,"");
if(idDigits && idDigits.length!==9) errs.push("ת.ז חייבת להיות בדיוק 9 ספרות");
if(updated.email){
  const emailOk = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(updated.email);
  if(!emailOk) errs.push("אימייל לא תקין");
}
if(!updated.assignedTo) errs.push("נציג מטפל חובה");
if(updated.firstName && updated.firstName.length<2) errs.push("שם פרטי קצר מדי");
if(updated.lastName && updated.lastName.length<2) errs.push("שם משפחה קצר מדי");

if(errs.length){
  toast("❌ " + errs[0], "warn");
  const f = (errs[0].includes("טלפון")) ? document.querySelector('#leadForm [name="phone"]')
          : (errs[0].includes("ת.ז")) ? document.querySelector('#leadForm [name="idNumber"]')
          : (errs[0].includes("אימייל")) ? document.querySelector('#leadForm [name="email"]')
          : (errs[0].includes("נציג")) ? document.querySelector('#leadForm [name="assignedTo"]')
          : (errs[0].includes("פרטי")) ? document.querySelector('#leadForm [name="firstName"]')
          : document.querySelector('#leadForm [name="lastName"]');
...
function getFilteredSortedLeads(){
  const q = (safeVal("q","")).trim().toLowerCase();
  const fStatus = safeVal("fStatus","");
  const fSource = safeVal("fSource","");
  const sortBy = safeVal("sortBy","newest");
  ...
  tbody.innerHTML = leads.map(l=>{
    ...
    return `
      <tr>
        ...
        <td>${escapeHTML(String(l.assignedTo || l.owner || "—"))}</td>
        <td>
          <div class="actions">
...
/* ===== Customer Dossier (Slim overrides) ===== */
function _cdFmtMoney(n){
  try{
    const v = Number(n)||0;
...
function openCustomerView(idOrLead){
  ...
  // Header
  const sub = document.getElementById("customerSub");
  if(sub) sub.textContent = `לקוח: ${lead.name||"—"} • ת.ז: ${lead.tz||"—"} • טלפון: ${lead.phone||"—"}`;

  // Stats
  const setText = (id, txt)=>{ const e=document.getElementById(id); if(e) e.textContent = txt; };
  setText("cd_name2", lead.name || "—");
  setText("cd_phone2", lead.phone || "—");
  setText("cd_tz2", lead.tz ? ("ת.ז: " + lead.tz) : "");
  setText("cd_rep2", (lead.rep || lead.agent || lead.owner || lead.assignedTo || "—"));

  const prem = Number(lead.premiumMonthly ?? lead.premium_monthly ?? 0) || 0;
  setText("cd_premiumMonthly2", prem ? _cdFmtMoney(prem) : "—");
  safeSet("cd_edit_premiumMonthly", prem ? prem : "");

  renderCustomerHistory();

  // Update contact buttons targets
  try{ wireCustomerContacts(lead); }catch(e){}
}
...
function renderCustomerHistory(){
  const lead = getCustomerLead();
  const box = document.getElementById("cd_history_box");
  if(!box) return;
  if(!lead){ box.innerHTML=""; return; }

  const events = (appState.events||[]).filter(ev=>ev && ev.leadId===lead.id).slice().reverse();
  if(!events.length){
    box.innerHTML = '<div class="event"><div class="t">אין היסטוריה עדיין</div><div class="d muted">כשיש שינויים/חתימות — זה יופיע כאן.</div></div>';
    return;
  }
  box.innerHTML = events.map(ev=>`
    <div class="event">
...
function wireCustomerContacts(lead){
  const phoneRaw = String(lead.phone||"").trim();
  const digits = phoneRaw.replace(/\D+/g,"");
  const phoneIL = digits.startsWith("0") ? ("972"+digits.slice(1)) : digits; // for wa.me
  const email = String(lead.email||"").trim();

  const bCall = document.getElementById("btnCustomerCall");
  if(bCall) bCall.onclick = ()=>{ if(digits) location.href = "tel:"+digits; else toast("אין טלפון ללקוח", "warn"); };

  const bWa = document.getElementById("btnCustomerWhatsApp");
  if(bWa) bWa.onclick = ()=>{ if(phoneIL) window.open("https://wa.me/"+phoneIL, "_blank"); else toast("אין טלפון לוואטסאפ", "warn"); };

  const bMail = document.getElementById("btnCustomerEmail");
  if(bMail) bMail.onclick = ()=>{ 
    if(email) location.href = "mailto:"+encodeURIComponent(email);
    else toast("אין אימייל ללקוח", "warn");
  };
}

function wireCustomerView(){
  const back = document.getElementById("btnCustomerBack");
  if(back) back.onclick = ()=> setView("leads");

  const savePrem = document.getElementById("btnCustomerSavePremium");
  if(savePrem) savePrem.onclick = ()=>{
    const lead = getCustomerLead();
    if(!lead) return;
    const prem = Number(safeVal("cd_edit_premiumMonthly","0"));
    lead.premiumMonthly = isFinite(prem) ? prem : 0;
    addEvent("premium", "עודכנה פרמיה חודשית", lead.id);
    saveState();
    toast("נשמר ✅", "success");
    openCustomerView(lead);
  };
  ...
  const signNow = ()=>{
    ...
    addEvent("sign", "נשלח מסמך לחתימה (דמו)", lead.id);
    saveState();
    const s = document.getElementById("cd_sign_status");
    if(s) s.textContent = "נשלח לחתימה ✅ (דמו)";
    toast("נשלח לחתימה ✅", "success");
    renderCustomerHistory();
  };

  const b1 = document.getElementById("btnSignSendNow");
  if(b1) b1.onclick = signNow;
  const b2 = document.getElementById("btnCustomerSendSign");
  if(b2) b2.onclick = signNow;
}
