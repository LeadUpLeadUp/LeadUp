...
*/
(function(){
  const DEFAULT_PASSWORD = "3316";            // <-- שנה כאן אם תרצה סיסמה אחרת
  ...
  // Wire UI (robust: works even if DOMContentLoaded already fired)
function wire(){
  const btn = $("#loginBtn");
  const inp = $("#loginPassword");
  ...
  if(inp && !inp.__wired){
    ...
    inp.addEventListener("keyup", onEnter);
  }

  if(tog && inp && !tog.__wired){
    tog.__wired = true;
