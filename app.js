...
/* ===== Customer full-page view (minimal) ===== */
let __customerId = null;

function _setText(id, val){
  const n = document.getElementById(id);
  if(!n) return;
  n.textContent = (val === null || val === undefined || val === "") ? "—" : String(val);
}
...
function openCustomerView(idOrLead){
  ...
  try{ _cdEnsureLeadCollections && _cdEnsureLeadCollections(lead); }catch(e){}

  // Switch view
  setView("customer");

  // Header
  _setText("customerTitle", "תיק לקוח");
  _setText("customerSub", `לקוח: ${lead.name || "—"} • ת.ז: ${lead.tz || lead.idNumber || "—"} • טלפון: ${lead.phone || "—"}`);

  // Top stats
  _setText("cd_name2", lead.name || "—");
  _setText("cd_phone2", lead.phone || "—");
  _setText("cd_tz2", (lead.tz || lead.idNumber) ? ("ת.ז: " + (lead.tz || lead.idNumber)) : "");
  _setText("cd_email2", lead.email || "—");

  // Minimal details
  _setText("cd_show_name", lead.name || "—");
  _setText("cd_show_phone", lead.phone || "—");
  _setText("cd_show_tz", (lead.tz || lead.idNumber) || "—");
  _setText("cd_show_email", lead.email || "—");
  _setText("cd_show_address", lead.address || "—");

  // Monthly premium (sum of policies premiums, if exists)
  let total = 0, count = 0;
  try{
    const arr = Array.isArray(lead.policies) ? lead.policies : [];
    count = arr.length;
    total = arr.reduce((s,p)=> s + (Number(p?.premium)||0), 0);
  }catch(e){}
  try{
    const v = (count>0) ? (_cdFmtMoney ? _cdFmtMoney(total) : String(total)) : "—";
    _setText("cd_premiumTotal2", v);
  }catch(e){
    _setText("cd_premiumTotal2", count>0 ? String(total) : "—");
  }
...
function renderCustomerDocs(){
  ...
  if(!lead) return;
  try{ _cdEnsureLeadCollections && _cdEnsureLeadCollections(lead); }catch(e){}
  const tb = document.getElementById("cd_docs_tbody");
  if(!tb) return;

  tb.innerHTML = (lead.docs||[]).map((d,idx)=>`
    <tr>
      <td><b>${escapeHTML(d?.name || "—")}</b></td>
      <td class="small muted" style="max-width:360px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escapeHTML(d?.url||"")}</td>
  ...
  tb.querySelectorAll("button[data-doc-open]").forEach(b=>{
    b.onclick = ()=>{
      ...
      if(!doc?.url) return;
      const fr = document.getElementById("cd_doc_frame");
  ...
  tb.querySelectorAll("button[data-doc-del]").forEach(b=>{
    b.onclick = ()=>{
      ...
      try{ addEvent && addEvent("doc", "נמחק מסמך", {leadId: lead.id}); }catch(e){}
      saveState();
      renderCustomerDocs();
    };
  });
...
function wireCustomerView(){
  document.getElementById("btnCustomerBack")?.addEventListener("click", ()=>{ setView("leads"); });

  // Focus add-document fields
  document.getElementById("btnCustomerAddDoc")?.addEventListener("click", ()=>{
    try{ document.getElementById("cd_doc_url")?.focus(); }catch(e){}
  });
  ...
  document.getElementById("btnDocAdd")?.addEventListener("click", ()=>{
    ...
    try{ addEvent && addEvent("doc", `נוסף מסמך: ${name}`, {leadId: lead.id}); }catch(e){}
    saveState();
    renderCustomerDocs();
  });
}
