  ...
  function openPrintView(proposal){
    ...
    try {
      ...
      const oldRows = oldPolicies.map(x => {
        const prem = nnum(x.premium);
        const discPct = nnum(x.discountPct ?? x.discountPercent ?? x.discount ?? 0);
        const after = discPct>0 ? (prem * (1 - discPct/100)) : prem;
        const discAmt = Math.max(0, prem - after);
        return `
        <tr>
          ...
          <td>${money(prem)}</td>
          <td>${discPct ? esc(discPct + '%') : '—'}</td>
          <td>${discAmt ? money(discAmt) : '—'}</td>
          <td>${money(after)}</td>
      ...
      `;
      }).join('') || `<tr><td colspan="10" class="mutedCell">אין פוליסות</td></tr>`;
      ...
      // Payment (full details: internal)
      function payerSectionHtml(){
        const pay = p.payer || null;
        if(!pay) return `<div class="muted">—</div>`;

        const method = String(pay.method || 'card');
        const override = pay.override || {};
        const ovOn = !!override.enabled;

        const rows = [];
        rows.push(`<tr><td>שיטה</td><td>${esc(method==='card'?'אשראי':(method==='hok'?'הו״ק':'אחר'))}</td></tr>`);

        if(method==='card'){
          const c = pay.card || {};
          rows.push(`<tr><td>שם על הכרטיס</td><td>${esc(c.name)}</td></tr>`);
          rows.push(`<tr><td>ת״ז משלם</td><td>${esc(c.tz)}</td></tr>`);
          rows.push(`<tr><td>4 ספרות אחרונות</td><td>${c.last4 ? esc('**** ' + c.last4) : '—'}</td></tr>`);
          rows.push(`<tr><td>תוקף</td><td>${esc(c.exp)}</td></tr>`);
        }else if(method==='hok'){
          const h = pay.hok || {};
          rows.push(`<tr><td>בנק</td><td>${esc(h.bank)}</td></tr>`);
          rows.push(`<tr><td>סניף</td><td>${esc(h.branch)}</td></tr>`);
          rows.push(`<tr><td>חשבון</td><td>${esc(h.account)}</td></tr>`);
          if(h.exp) rows.push(`<tr><td>תוקף</td><td>${esc(h.exp)}</td></tr>`);
        }

        if(ovOn){
          rows.push(`<tr><td colspan="2" class="subHead">משלם חריג (לא המבוטח הראשי)</td></tr>`);
          rows.push(`<tr><td>שם פרטי</td><td>${esc(override.firstName)}</td></tr>`);
          rows.push(`<tr><td>שם משפחה</td><td>${esc(override.lastName)}</td></tr>`);
          rows.push(`<tr><td>ת״ז</td><td>${esc(override.tz)}</td></tr>`);
          rows.push(`<tr><td>תאריך לידה</td><td>${esc(override.birthDate)}</td></tr>`);
          rows.push(`<tr><td>קשר למבוטח</td><td>${esc(override.relationship)}</td></tr>`);
        }

        return `
          <div class="box">
            <div class="boxTitle">פרטי משלם</div>
            <table class="kv">
      ...
      // Full personal details (everything the rep typed)
      function insuredPersonalBox(insId){
        const raw = (insId==='main') ? (p.customer||{}) : ((p.insuredDetails && p.insuredDetails[insId]) ? p.insuredDetails[insId] : {});
        if(!raw) return '';
        ...
        const labels = [
          ['סוג מבוטח', labelInsured(insId)],
          ['שם מלא', name],
          ['ת״ז', d.tz],
          ['תאריך לידה', d.birthDate],
          ['מין', d.gender],
          ['מצב משפחתי', d.maritalStatus],
          ['נייד', d.mobile || d.phone],
          ['אימייל', d.email],
          ['כתובת', d.addressFull],
          ['עיר', d.city],
          ['מיקוד', d.zip],
        ...
        const bmi = (h > 0 && wkg > 0) ? (wkg / Math.pow(h/100, 2)) : 0;
        const bmiText = bmi ? (Math.round(bmi*10)/10).toFixed(1) : '';
        if(bmiText) labels.push(['BMI', bmiText]);

        // Notes (keep at end)
        if(d.notes && String(d.notes).trim()){
          labels.push(['הערות', String(d.notes).trim()]);
        }

        // Include any extra fields we don't know (but skip internal/empty)
        const knownKeys = new Set(['fullName','firstName','lastName','tz','birthDate','gender','maritalStatus','mobile','phone','email','addressFull','city','zip','hmo','shaban','occupation','tzIssueDate','heightCm','weightKg','notes']);
        ...
        return `
          <div class="box">
            <div class="boxTitle">פרטים אישיים — ${esc(labelInsured(insId))}</div>
            <table class="kv"><tbody>${rows}</tbody></table>
          </div>
        `;
      }

      const insuredPersonalBoxes = insuredList.map(ins => insuredPersonalBox(ins.id)).filter(Boolean).join('');
// Logo (text-based, premium)
      ...
      <head>
        ...
        <style>
          ...
          *{ box-sizing:border-box; }
          ...
          table.t th, table.t td{ word-break:break-word; 
          ...
          .kv td{ border-top:1px solid var(--line); padding:8px 10px; vertical-align:top; font-size:12px; }
          .kv td:first-child{ width:26%; color:var(--muted); font-weight:700; }
          .subHead{ background:rgba(202,167,74,.10); border-top:1px solid var(--goldSoft); font-weight:900; color:var(--ink); }
          .box{ border:1px solid var(--line); border-radius:16px; padding:10px 12px; margin:10px 0; background:#fff; }
      ...
      <body>
        <div class="page">
          ...
          ${insuredPersonalBoxes ? `<h2>פרטים אישיים (כמו שהוזן במערכת)</h2>${insuredPersonalBoxes}` : ``}

          <h2>פוליסות קיימות</h2>
          <table class="t">
            <thead><tr><th>מבוטח</th><th>חברה</th><th>מוצר</th><th>מס׳ פוליסה</th><th>פרמיה</th><th>הנחה</th><th>סכום הנחה</th><th>אחרי הנחה</th><th>סכום ביטוח</th><th>סכום פיצוי</th></tr></thead>
            <tbody>
              ${oldRows}
              <tr class="sumRow">
                <td colspan="4">סה״כ</td>
                <td class="sumGold">${money(sumOld)}</td>
                <td colspan="5"></td>
              </tr>
            </tbody>
          </table>
