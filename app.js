...
function seedDemoDataOnce(){
  ...
  try{
    ...
    // Fallback: local save + best-effort render (never break the app)
    try{ localStorage.setItem(LS_KEY, JSON.stringify(appState)); }catch(_e){}
    try{ if(typeof renderAll === "function") renderAll(); }catch(_e){}
  }
  try{ toast("נטענו נתוני דמו ✅", "success"); }catch(_e){}
...
/* =======================
   ...
   ======================= */
(async function init(){
  // Seed demo data only in a fresh local environment (safe + idempotent)
  try{ seedDemoDataOnce(); }catch(e){}

  if(!(appState.events||[]).length){
    try{ addEvent("init", "המערכת עלתה • מצב LocalStorage"); }catch(e){}
