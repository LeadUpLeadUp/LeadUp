...
nav.addEventListener("click", (e)=>{
  ...
  if(!btn) return;
  if(btn.dataset && btn.dataset.page){ window.location.href = btn.dataset.page; return; }
  setView(btn.dataset.view);
});
function setView(view){
  // "esign" is a modal flow (not a full page)
  ...
  const titles = {
    ...
    timeline: ["היסטוריה", "Timeline של כל פעולה ושינוי במערכת"],
...
if(_q){
  ...
  _q.addEventListener("input", ()=>{
    clearTimeout(t);
    t = setTimeout(()=>{ try{ renderLeads(); }catch(e){} }, 80);
  });
}

/* ===== Buttons ===== */
// Top quick button: open "My Flows / My Clients" area (not a task modal)
const _btnQuickTask = el("btnQuickTask");
if(_btnQuickTask){
  _btnQuickTask.addEventListener("click", ()=>{
    if(typeof window.setView === "function") window.setView("myflows");
    else setView("myflows");
  });
}
el("btnNewLead")?.addEventListener("click", ()=> openLeadForm(null));
...
function getFilteredSortedLeads(){
  ...
  const sortBy = el("sortBy")?.value || "hot";
