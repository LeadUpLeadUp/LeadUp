  ...
  function renderOldPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="card" style="margin-top:14px">
        <div class="sectionTitle" style="margin:0 0 10px">סיבת החלפה פוליסה בתיק הביטוחי</div>
        <div class="field" style="margin:0">
          <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px">
            ${(()=>{
              const selected = ((p.oldPolicyReasons||{})[insuredId]||[]);
              const items = ["סדר בתיק הביטוחי", "הוזלת עלויות/מיקסום זכויות", "להבין זכויות", "להבין אם קיים עודף/חוסר", "התאמת סיכונים", "ריכוז תיק", "בדיקה מול אתר הר הביטוח"];
              return items.map((t,ii)=>{
                const checked = selected.includes(t) ? 'checked' : '';
                return `<label class="chipCheck" style="display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid rgba(18,19,25,.10);border-radius:14px;background:rgba(255,255,255,.75)">
                  <input type="checkbox" data-old-reason="${escapeHtml(t)}" ${checked} style="width:18px;height:18px">
    ...
    // reasons checkboxes
    const reasonInputs = wizardMain.querySelectorAll('input[type="checkbox"][data-old-reason]');
    reasonInputs.forEach(inp => {
      inp.addEventListener('change', () => {
        const t = inp.getAttribute('data-old-reason') || '';
        p.oldPolicyReasons = p.oldPolicyReasons || {};
        const arr = Array.isArray(p.oldPolicyReasons[insuredId]) ? p.oldPolicyReasons[insuredId] : [];
        const has = arr.includes(t);
        if(inp.checked && !has) arr.push(t);
        if(!inp.checked && has) arr.splice(arr.indexOf(t),1);
        p.oldPolicyReasons[insuredId] = arr;
        saveLocal();
      });
    });
...
`;
  ...
  function renderSummaryStep(){
    ...
    const oldReasonsBlock = (()=> {
      const reasonsMap = (p.oldPolicyReasons || {});
      const list = (p.insuredList||[]).filter(ins => insuredHasAnyData(p, ins.id));
      const blocks = list.map(ins => {
        const arr = Array.isArray(reasonsMap[ins.id]) ? reasonsMap[ins.id] : [];
        if(!arr.length) return '';
        return `<div class="card" style="margin:10px 0 0">
          <div class="sectionTitle" style="margin:0 0 8px">סיבת החלפה פוליסה בתיק הביטוחי – ${escapeHtml(ins.label||labelInsured(ins.id))}</div>
          <ul style="margin:0;padding:0 18px">
            ${arr.map(t=>`<li style="margin:6px 0">${escapeHtml(t)}</li>`).join('')}
          </ul>
        </div>`;
      }).join('');
      return blocks || '';
            ...
            <table>
              ...
              <tbody>
                ${rows.map(x => {
                  ...
                  return `<tr><td>${esc(x.company)}</td><td>${esc(x.product)}</td><td>${money(x.premium||0)}</td></tr>`;
                }).join('')}
              </tbody>
            </table>
          </div>
