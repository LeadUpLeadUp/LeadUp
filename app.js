(() => {
  ...
  // Floating bottom-left action (CTA) — keeps the main "המשך..." button accessible while scrolling
  let floatingActionBar = document.getElementById("floatingActionBar");
  if (!floatingActionBar){
    floatingActionBar = document.createElement("div");
    floatingActionBar.id = "floatingActionBar";
    floatingActionBar.className = "floatingActionBar hidden";
    document.body.appendChild(floatingActionBar);
  }

  function setFloatingAction(content){
    ...
    if (typeof content === "string") {
      floatingActionBar.innerHTML = content;
    } else {
      floatingActionBar.appendChild(content);
    }
    floatingActionBar.classList.remove("hidden");
  }
  function clearFloatingAction(){
    setFloatingAction(null);
  }

  // Smart Top Bar (Control Bar)
  const cbUser    = $("cbUser");
  const cbDot     = $("cbDot");
  const cbStatus  = $("cbStatus");
  const cbMode    = $("cbMode");
  const cbSavedAt = $("cbSavedAt");

  const tbNewCase   = $("tbNewCase");
  const tbSaved     = $("tbSaved");
  const tbSettings  = $("tbSettings");

  function getDisplayUser(){
    // optional: you can set this key from future login/users module
    const v = (localStorage.getItem("GI_CURRENT_USER") || localStorage.getItem("LEADCORE_CURRENT_USER") || "").trim();
    return v || "מנהל";
  }
  ...
  function updateControlBar(){
    if (cbUser) cbUser.textContent = `נציג: ${getDisplayUser()}`;

    // Mode / connection hint
    const mode = state?.settings?.storageMode || "local";
    const isServer = mode === "server";

    if (cbMode) cbMode.textContent = isServer ? "Google Sheets" : "מקומי";

    // Status: until server sync exists, we show readiness based on mode
    if (cbStatus){
      cbStatus.textContent = isServer ? "מצב שרת (בקרוב)" : "מוכן לעבודה";
    }

    if (cbDot){
      cbDot.classList.remove("ok","warn","bad");
      cbDot.classList.add(isServer ? "warn" : "ok");
    }

    // Last saved time (current case)
    const savedIso = state?.case?.savedAtISO || state?.case?.createdAtISO || "";
    if (cbSavedAt) cbSavedAt.textContent = `עודכן: ${fmtHe(savedIso)}`;
  }

  // Control Bar actions (safe)
  tbNewCase?.addEventListener("click", ()=>{
  ...
  tbSaved?.addEventListener("click", ()=> setView("saved"));
  tbSettings?.addEventListener("click", ()=> setView("settings"));
  ...
  const SIDEBAR_COLLAPSED_KEY = "GI_SIDEBAR_COLLAPSED"; // "1" collapsed, "0" expanded

  function applySidebarCollapsed(isCollapsed){
    document.body.classList.toggle("sidebar-collapsed", !!isCollapsed);
    if (sidebarToggle){
      // aria-expanded refers to the controlled region being expanded (true when NOT collapsed)
...
const state = {
  ...
  function updateSide(){
    ...
    if (stageBadge) stageBadge.textContent = stepName;
    if (badgeStage) badgeStage.textContent = stepName;

    if (badgeProducts) badgeProducts.textContent = String(state.case.products.length);
    if (badgeTotal) badgeTotal.textContent = String(premiumTotal());
    if (badgeSecondary) badgeSecondary.textContent = state.case.hasSecondary ? "כן" : "לא";
    if (badgeChildren) badgeChildren.textContent = String(state.case.children.length);

    if (caseIdChip) caseIdChip.textContent = state.case.caseId;
    if (caseTitle) caseTitle.textContent = buildCaseTitle();
    if (caseStatusChip) caseStatusChip.textContent = state.case.status;

    const miss = missingCritical();

    // Update status even if "בקרה" panel removed
    if (miss.length === 0){
    ...
    // Render missing list only if side panel exists
    if (missingList){
...
function renderInsuredComparisonSummary(){
  ...
  function renderDetailsActions(){
    // Keep the main CTA always accessible (bottom-left), like a real CRM
    setFloatingAction(`
      <button class="btn primary" id="btnNextToCancellations" type="button">המשך לביטול פוליסות</button>
    `);

    const btn = floatingActionBar.querySelector("#btnNextToCancellations");
...
<body>
  ...
  function renderSaved(){
    ...
    if (stageBadge) stageBadge.textContent = "תיקים";
    if (badgeStage) badgeStage.textContent = "תיקים";
    ...
    // Side panel summary
    if (badgeProducts) badgeProducts.textContent = String(filtered.length);
    if (badgeTotal) badgeTotal.textContent = "—";
    if (badgeSecondary) badgeSecondary.textContent = "—";
    if (badgeChildren) badgeChildren.textContent = "—";
    if (missingList) missingList.innerHTML = `<div class="chip">תוצאות: <span class="mono">${filtered.length}</span> מתוך <span class="mono">${list.length}</span></div>`;
  ...
  function renderSettings(){
    ...
    if (stageBadge) stageBadge.textContent = "הגדרות";
    if (badgeStage) badgeStage.textContent = "הגדרות";
