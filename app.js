...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  function render(){
    ...
    if(route === 'jofi'){
      pageTitleEl.textContent = "ג׳ופי";
      crumbEl.textContent = "Dashboard";
...
function debounce(fn, wait){
  ...
  // ---------- JOFI Dashboard (ג׳ופי) ----------
  const JOFI_CFG_KEY = 'GI_JOFI_CFG_V1';

  function loadJofiCfg_(){
    try{
      const raw = localStorage.getItem(JOFI_CFG_KEY);
      if(raw){
        const j = JSON.parse(raw);
        return {
          monthlyTarget: +j.monthlyTarget || 2500,
          cancelLimit: +j.cancelLimit || 1500,
          dailyTarget: +j.dailyTarget || 0
        };
      }
    }catch(e){}
    return { monthlyTarget: 2500, cancelLimit: 1500, dailyTarget: 0 };
  }

  function saveJofiCfg_(cfg){
    try{ localStorage.setItem(JOFI_CFG_KEY, JSON.stringify(cfg||{})); }catch(e){}
  }
  ...
  function sumPremium_(arr, field){
    return (Array.isArray(arr) ? arr : []).reduce((acc, x) => acc + (parseFloat((x && x[field]) || '0') || 0), 0);
  }

  function calcCancelledPremium_(proposal){
    const oldArr = proposal && Array.isArray(proposal.oldPolicies) ? proposal.oldPolicies : [];
    let total = 0;
    oldArr.forEach(item => {
      if(!item) return;
      const dec = String(item.decision || item.old_decision || '').trim();
      if(dec === 'cancel_full' || dec === 'cancel_partial'){
        // Prefer explicit cancelled amount if exists (future-proof), otherwise fall back to policy premium
        const explicit = parseFloat(item.cancelPremium || item.old_cancel_premium || '') || 0;
        const prem = parseFloat(item.premium || item.old_premium || '') || 0;
        total += (explicit > 0 ? explicit : prem);
      }
    });
    return total;
  }

  function getAgentsList_(){
    const set = new Set();
    (state.teamAgents||[]).forEach(a => a && set.add(a));
    (state.customers||[]).forEach(c => c && c.assignedAgent && set.add(c.assignedAgent));
    if(state.agentName) set.add(state.agentName);
    return Array.from(set).filter(Boolean).sort((a,b)=>a.localeCompare(b,'he'));
  }

  function computeJofiStats_(agentName){
    const now = new Date();
    const mk = monthKey_(now.toISOString());
    const dk = dateKey_(now.toISOString());

    const rel = (state.proposals||[]).filter(p => String(p.assignedAgent||'') === String(agentName||''));

    const relMonth = rel.filter(p => monthKey_(p.createdAt) === mk);
    const relDay = rel.filter(p => dateKey_(p.createdAt) === dk);

    const soldMonth = relMonth.reduce((acc,p)=> acc + sumPremium_(p.newPolicies,'premium') + sumPremium_(p.newPolicies,'new_premium'), 0);
    const soldDay   = relDay.reduce((acc,p)=> acc + sumPremium_(p.newPolicies,'premium') + sumPremium_(p.newPolicies,'new_premium'), 0);

    const cancelledMonth = relMonth.reduce((acc,p)=> acc + calcCancelledPremium_(p), 0);
    ...
    return {
      ...
      soldDay,
      cancelledMonth,
      proposalsMonth: relMonth.length
    };
  }

  function clamp01_(x){ x = +x || 0; if(x<0) return 0; if(x>1) return 1; return x; }

  function gaugeHTML_(opt){
    const id = opt.id;
    const title = opt.title;
    const value = +opt.value || 0;
    const target = +opt.target || 0;
    const caption = opt.caption || '';
    const mode = opt.mode || 'up'; // 'up' (higher is better) or 'down' (lower is better)
    const pctRaw = (target>0) ? (value/target) : 0;
    const pct = clamp01_(pctRaw);
    const needleDeg = -90 + (pct*180);

    let level = 'bad';
    if(mode === 'down'){
      // value low = good
      if(pct <= 0.4) level = 'good';
      else if(pct <= 0.75) level = 'warn';
      else level = 'bad';
    }else{
      if(pct >= 0.9) level = 'good';
      else if(pct >= 0.6) level = 'warn';
      else level = 'bad';
    }

    const fmt = (n)=> (Math.round((+n||0)*100)/100).toLocaleString('he-IL');
    return `
      <div class="jofiCard ${level}" id="${id}">
        <div class="jofiHead">
          <div class="jofiTitle">${title}</div>
          <div class="jofiSub">${caption}</div>
        </div>
        <div class="jofiGaugeWrap">
          <svg class="jofiGauge" viewBox="0 0 200 120" aria-hidden="true">
            <path class="jofiTrack" pathLength="100" d="M20,110 A80,80 0 0 1 180,110" />
            <path class="jofiValue" pathLength="100" data-pct="${pct}" d="M20,110 A80,80 0 0 1 180,110" />
            <g class="jofiNeedle" style="transform: rotate(${needleDeg}deg); transform-origin: 100px 110px;">
              <line x1="100" y1="110" x2="165" y2="110" />
            </g>
            <circle class="jofiHub" cx="100" cy="110" r="6" />
          </svg>
        </div>
        <div class="jofiNumbers">
          <div class="jofiVal"><span>ביצוע</span><strong>₪ ${fmt(value)}</strong></div>
          <div class="jofiTar"><span>${mode==='down'?'תקרה':'יעד'}</span><strong>₪ ${fmt(target)}</strong></div>
        </div>
      </div>
    `;
  }

  function renderKpisJofi(){
    // KPI strip at top: show selected agent + month summary
    const cfg = loadJofiCfg_();
    const daily = cfg.dailyTarget > 0 ? cfg.dailyTarget : Math.round((cfg.monthlyTarget||2500) / 22);
    const agents = getAgentsList_();
    const sel = (state._jofiSelectedAgent && agents.includes(state._jofiSelectedAgent)) ? state._jofiSelectedAgent : state.agentName;

    const s = computeJofiStats_(sel);

    kpisEl.innerHTML = `
      <div class="kpi">
        <div class="kpiLabel">חודש</div>
        <div class="kpiValue">${s.monthKey}</div>
      </div>
      <div class="kpi">
        <div class="kpiLabel">פרמיה חודשית (סה״כ)</div>
        <div class="kpiValue">₪ ${(Math.round(s.soldMonth)).toLocaleString('he-IL')}</div>
      </div>
      <div class="kpi">
        <div class="kpiLabel">ביטולים החודש</div>
        <div class="kpiValue">₪ ${(Math.round(s.cancelledMonth)).toLocaleString('he-IL')}</div>
      </div>
      <div class="kpi">
        <div class="kpiLabel">נציג</div>
        <div class="kpiValue">
          ${isAdmin_() ? `
            <select id="jofiAgentSel" class="input" style="height:36px; padding:6px 10px; min-width: 180px;">
              ${agents.map(a => `<option value="${esc(a)}" ${a===sel?'selected':''}>${esc(a)}</option>`).join('')}
            </select>
          ` : esc(state.agentName)}
        </div>
      </div>
    ...
    // Admin can also adjust targets quickly (lightweight, not mandatory)
    if(isAdmin_()){
      const wrap = document.createElement('div');
      wrap.className = 'jofiCfgMini';
      wrap.innerHTML = `
        <div class="jofiCfgRow">
          <label>יעד חודשי</label>
          <input id="jofiTMonth" class="input" type="number" value="${cfg.monthlyTarget||2500}" />
        </div>
        <div class="jofiCfgRow">
          <label>תקרת ביטולים</label>
          <input id="jofiTCancel" class="input" type="number" value="${cfg.cancelLimit||1500}" />
        </div>
        <div class="jofiCfgRow">
          <label>יעד יומי</label>
          <input id="jofiTDay" class="input" type="number" value="${daily}" />
        </div>
        <button class="btn btnSoft" id="jofiSaveCfg">שמור יעדים</button>
      `;
      kpisEl.appendChild(wrap);

      $('#jofiSaveCfg')?.addEventListener('click', () => {
        const m = parseFloat($('#jofiTMonth')?.value||'') || 2500;
        const c = parseFloat($('#jofiTCancel')?.value||'') || 1500;
        const d = parseFloat($('#jofiTDay')?.value||'') || Math.round(m/22);
        saveJofiCfg_({ monthlyTarget:m, cancelLimit:c, dailyTarget:d });
        toast('נשמר', 'יעדי ג׳ופי עודכנו');
        render();
      });
    }

    $('#jofiAgentSel')?.addEventListener('change', (e) => {
      state._jofiSelectedAgent = e.target.value;
      render();
    });
  }

  function renderJofiDashboard(){
    const cfg = loadJofiCfg_();
    const monthlyTarget = cfg.monthlyTarget || 2500;
    const cancelLimit = cfg.cancelLimit || 1500;
    const dailyTarget = (cfg.dailyTarget && cfg.dailyTarget>0) ? cfg.dailyTarget : Math.round(monthlyTarget/22);
    ...
    const cards = [
      gaugeHTML_({
        ...
        mode:'up',
        caption:`יעד בסיסי: ₪ ${monthlyTarget.toLocaleString('he-IL')} לחודש`
      }),
      gaugeHTML_({
        id:'gCancel',
        title:'מד ביטולים (חודשי)',
        value:s.cancelledMonth,
        target:cancelLimit,
        mode:'down',
        caption:`לא לעבור יותר מ־₪ ${cancelLimit.toLocaleString('he-IL')} בחודש`
      }),
      gaugeHTML_({
        id:'gSoldDay',
        title:'מד יעד יומי',
        ...
        target:dailyTarget,
        mode:'up',
        caption:`סך פרמיה שנמכרה היום (${s.dateKey})`
      }),
    ].join('');
    ...
    viewEl.innerHTML = `
      <div class="jofiWrap">
        ...
        <div class="jofiNotes">
          <div class="noteTitle">איך זה מחושב</div>
          <ul>
            <li><b>חודשי:</b> סכום הפרמיות של פוליסות חדשות בכל ההצעות שנפתחו החודש לנציג.</li>
            <li><b>יומי:</b> סכום הפרמיות של פוליסות חדשות בכל ההצעות שנפתחו היום לנציג.</li>
            <li><b>ביטולים:</b> סכום הפרמיות של פוליסות בתיק הביטוחי שסומנו "ביטול מלא" / "ביטול חלקי" בהצעות החודש (נלקח מהשדה פרמיה של הפוליסה).</li>
          </ul>
        </div>
      </div>
    `;
    ...
    $$('.jofiValue', viewEl).forEach(p => {
      const pct = clamp01_(parseFloat(p.getAttribute('data-pct')||'0')||0);
      const len = 100; // uses pathLength=100
      p.style.strokeDasharray = `${len}`;
      p.style.strokeDashoffset = `${len - (len*pct)}`;
    });
  }
