window.LEADUP_AUTH = window.LEADUP_AUTH || {};
...
/* ===== State Migration (legacy keys) ===== */
function migrateLegacyState(){
  try{
    // If current key already exists, nothing to do
    if(localStorage.getItem(LS_KEY)) return;

    const legacyKeys = [
      "leadup_state", "leadup", "leadup_v0", "leadup_v2",
      "leadup_v1_state", "LEADUP_STATE", "crm_state"
    ];
...
/* ===== Global error handler (prevents silent failure) ===== */
window.addEventListener("error", function(ev){
  try{
    const e = document.getElementById("loginError");
    // Only show if login gate is visible
    const gate = document.getElementById("loginGate");
    if(gate && gate.style.display !== "none" && e){
      e.textContent = "שגיאה בטעינת המערכת (בדוק Console).";
    }
  }catch(_){}
});
